/**
 * @gateflow/tokens - scripts/check-contrast.ts
 * Automated WCAG 2.2 AA Contrast Verification Script.
...
const LIGHT_PALETTE = {
  ...
  textBrand: '#ed4b00',   // text-brand (Kimchi)
  textSuccess: '#059669', // text-success
  textWarning: '#d97706', // text-warning
  textDanger: '#dc2626',  // text-danger
  textAiLab: '#7c3aed',   // text-ai-lab
  borderSubtle: '#e2e6eb',
  borderBold: '#cbd2db',
};
...
function runAudit() {
  ...
  for (const test of tests) {
    ...
    const target = '>= ' + test.minRatio + ':1';

    console.log(`[${test.theme}] ${icon} ${test.fgName} on ${test.bgName} -> Ratio: ${formattedRatio} (Target: ${target})`);
