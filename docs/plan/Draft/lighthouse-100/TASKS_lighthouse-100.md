...
- [ ] Run automated Lighthouse audits across all 5 web apps (`marketing`, `design-system`, `resident-portal`, `client-dashboard`, `admin-dashboard`) to record baseline scores.
- [ ] Profile bundle sizes with `@next/bundle-analyzer` or manual chunk inspection.
- [ ] Create `@gateflow/ui/performance` utilities (dynamic island loader, Skeleton matching wrappers, image preloading hooks).
- [ ] Verify `next/font` configuration across monorepo for Inter and Cairo font metric overrides.
