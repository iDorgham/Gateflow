...
- **Steps**:
  1. Define semantic tokens in `packages/ui/src/globals.css`.
  2. Map `--background` to Anti-Flash White (`#F2F3F4`) and `--foreground` to 
     Midnight Blue (`#020035`).
  3. Map `--primary` to Kimchi Orange (`#ED4B00`).
  4. Update `LoginShell` and `DashboardShell` to use the new Midnight Blue 
     sidebar theme.
- **Acceptance Criteria**:
...
- **Steps**:
  1. Extend `GateAssignment` in `schema.prisma` with optional `startTime` 
     and `endTime`.
  2. Implement an API route for Project Aggregates (`contactsCount`, 
     `qrUsage`, `weeklyScans`).
  3. Create specialized Prisma helpers to calculate relative growth 
...
- **Goal**: Create the reusable "Edit" utility and the base TanStack 
  Table v8 engine.
- **Steps**:
  1. Implement a slide-from-right `EditPanel` component with a dimming overlay.
  2. Scaffold the `DataTable` base component using TanStack Table v8.
  3. Add support for server-side sorting, pagination, and multi-field 
     global search.
- **Acceptance Criteria**:
...
- **Steps**:
  1. Update `/dashboard/projects` list to use high-density cards with 
     KPI sparklines.
  2. Create `/[locale]/dashboard/projects/[projectId]/page.tsx` with 
     a rich hero header.
  3. Integrate KPI cards (Contacts, Units, Active QRs, Scans) into the header.
- **Acceptance Criteria**:
...
- **Goal**: Full CRM lifecycle management for project-scoped contacts 
  and units.
- **Steps**:
  1. Implement the "Contacts" and "Units" tabs within the Project detail page.
  2. Use the `EditPanel` for creating/editing Contacts and Units.
  3. Implement bulk actions (Export CSV, Bulk Delete).
- **Acceptance Criteria**:
