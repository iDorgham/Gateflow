...
- **Objective:** Expose **`pnpm prisma db seed`** (or package script) flags: `--dry-run`, `--test-integrity`, `--organizations.min/max`, etc.; print **console summary** (counts per model, duration); document ops in `docs/` (minimal: one section under `docs/reference/architecture/` or `docs/guides/` — only if no suitable file; prefer updating existing seed/guide doc).
...
- Move plan folder per `docs/development/PLAN_LIFECYCLE.md` when all phases merged.
