...
- **Refs**:
  ...
  - `docs/archive/legacy/PRD.md` — canonical PRD (exists)
...
Bring `docs/README.md` in sync with the actual current state, confirm `PRD.md` is the single canonical PRD, and verify the archive boundary is clean. No code changes.
...
- **Update `docs/README.md`**:
  ...
  - Add a **"How to contribute docs"** note: PRD changes go in `PRD.md`; new initiatives start with `/idea` → `/plan` → `/dev`.
- **Validate PRD**: Open `docs/archive/legacy/PRD.md` and confirm it does not reference a v5 PRD as "current" or contain unresolved TODOs that would mislead a new reader.
...
4. Open `docs/archive/legacy/PRD.md`, scan first 50 and last 50 lines for any unresolved TODOs or v5 cross-references. Note any found — no changes required if clean.
...
- [ ] PRD.md has no unresolved `TODO:` markers visible in a quick scan.
