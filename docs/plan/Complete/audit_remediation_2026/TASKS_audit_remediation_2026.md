...
- [x] Rotate affected secrets/credentials and record an operational receipt. _(Pending ops receipt — see phase log Residual Risks / Operational follow-up.)_
...
- [x] Inventory high-risk routes and required auth/RBAC/tenant/schema/rate/audit controls.
- [x] Consolidate composable controls and add negative/cross-tenant/replay tests.
- [x] Verify CSP/security headers across web apps.
- [x] Resolve Jest open handles and remove `--forceExit`, or approve dated exceptions.
- [x] Align architecture/security docs with actual stack and controls.
- [x] Run clean full typecheck, scanner gates, preflight, and re-test P0/P1 findings.
- [x] Reconcile related-plan duplication/backlog; write phase 4 log and final summary.

## Final gate

- [x] Every deferral has an owner, reason, expiry, and follow-up.
- [x] `pnpm typecheck:all` and `pnpm preflight` pass.
- [x] No verified P0/P1 finding remains reproducible.
- [x] Backlog path/status matches lifecycle.
