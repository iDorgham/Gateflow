...
- [ ] **2.1** Add React Query + React Table deps in `client-dashboard`
- [ ] **2.2** Build shared `ResidentsFilterBar` with URL sync
- [ ] **2.3** Create `useContacts(filters)` hook
- [ ] **2.4** Refactor Contacts table with React Table columns + aggregates
- [ ] **2.5** Add tag column inline add/remove + bulk tag actions
- [ ] **2.6** Add contacts table customizer (order/visibility) + persist view
- [ ] **2.7** Add "View Units" row action modal/sheet
