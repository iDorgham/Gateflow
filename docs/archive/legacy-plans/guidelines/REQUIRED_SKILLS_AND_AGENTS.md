...
The GateFlow team uses multiple AI-assisted tools. See `docs/development/guidelines/DEVELOPMENT_TOOLS.md` for the full reference.

| Tool                | Type | When to use                                                              |
| ------------------- | ---- | ------------------------------------------------------------------------ |
| **Cursor**          | IDE  | Main editor; inline AI, MCP, subagents; `.cursor/rules/` and skills      |
| **Kiro**            | IDE  | Domain skills, steering rules, automation hooks (lint, prisma, security) |
| **Antigravity IDE** | IDE  | Alternative AI IDE; parallel work on different tasks                     |
| **Claude CLI**      | CLI  | Quick terminal Q&A, scripting, batch analysis                            |
| **Opencode CLI**    | CLI  | Code generation and transformations from the command line                |
| **Gemini CLI**      | CLI  | Terminal-based Gemini; alternative model for certain tasks               |
| **Kilo CLI**        | CLI  | Lightweight terminal AI workflows and prompts                            |

All tools should follow the same repo rules (pnpm, multi-tenancy, QR security, no secrets in git). See `CLAUDE.md` and `docs/development/guidelines/AI_SKILLS_SUBAGENTS_RULES.md`.
...
#### 1. gf-conventions.md (Always Active)
...
**Trigger:** File edited (_.ts, _.tsx, _.js, _.jsx)  
