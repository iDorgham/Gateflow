# Symbol Catalog: packages-types

Auto-generated symbol index for planning and impact analysis.

- Base path: `packages/types/src`
- Source files indexed: 17

## `packages/types/src/auth.ts`

- Exports (6): `AuthUser`, `JwtPayload`, `LoginPayload`, `RefreshTokenPayload`, `RegisterPayload`, `TokenResponse`

## `packages/types/src/base.ts`

- Exports (19): `Gate`, `MaintenanceCategory`, `MaintenanceLocationType`, `MaintenancePriority`, `MaintenanceStatus`, `Organization`, `OrganizationType`, `Permission`, `Plan`, `Project`, `QRCode`, `QRCodeType`, `Role`, `ScanLog`, `ScanStatus`, `User`, `UserRoleType`, `WorkOrder`, `WorkOrderWithRelations`

## `packages/types/src/crm.ts`

- Exports (7): `CreateContactInput`, `CreateUnitInput`, `CrmContact`, `CrmPaginatedResponse`, `CrmUnit`, `UpdateContactInput`, `UpdateUnitInput`

## `packages/types/src/gate.ts`

- Exports (1): `GateWithRelations`

## `packages/types/src/maintenance.ts`

- Exports (4): `CreateWorkOrderInput`, `UpdateWorkOrderInput`, `WorkOrderQuery`, `isValidStatusTransition`

## `packages/types/src/organization-features.ts`

- Exports (6): `DashboardConfig`, `OrganizationFeatures`, `OrganizationTerminology`, `SettingsConfig`, `SidebarConfig`, `getOrganizationFeatures`

## `packages/types/src/organization.ts`

- Exports (1): `OrganizationWithRelations`

## `packages/types/src/qr-payload.ts`

- Exports (4): `QRPayload`, `QRRejectReason`, `QRVerificationResult`, `SignedQRString`

- Exports (2): `signQRPayload`, `verifyQRSignature`
+ Exports (4): `base64urlDecode`, `base64urlEncode`, `signQRPayload`, `verifyQRSignature`
- Exports (2): `signQRPayload`, `verifyQRSignature`

## `packages/types/src/qr-validate.ts`

- Exports (3): `QRValidateRejectReason`, `QRValidateRequest`, `QRValidateResponse`

## `packages/types/src/qr.ts`

- Exports (1): `QRCodeWithRelations`

## `packages/types/src/scan-event.ts`

- Exports (7): `AuditTrailEntry`, `BulkScanRequest`, `BulkScanResponse`, `QueuedScan`, `ScanEvent`, `SyncPayload`, `SyncResponse`

## `packages/types/src/scan-log.ts`

- Exports (1): `ScanLogWithRelations`

## `packages/types/src/user.ts`

- Exports (1): `UserWithOrganization`
