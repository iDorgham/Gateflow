...
- **Visible slash commands (Cursor)**: `/brainstorm`, `/idea`, `/draft`, `/prompt`, `/plan`, `/dev`, `/ship`, `/guide`
  ...
  - `/idea`: Refine goals and capture context into `docs/development/initiatives/IDEA_<slug>.md` and backlog entries in `docs/plan/backlog/ALL_TASKS_BACKLOG.md`.
  - `/plan`: Turn an idea or `/prompt` output into a phased plan in `docs/plan/Draft/<slug>/`; use `/plan ready <slug>` to move to `Ready/` when approved.
  - `/draft`: Capture raw intent in `docs/plan/Draft/<slug>/` before formal planning (`DRAFT_<slug>.md`).
  - `/prompt`: Emit `FOR_PLAN_PROMPT.md` from a draft for paste into `/plan <slug>`.
  - `/dev`: Implement **one phase** end‑to‑end using `phases/NN_<title>/PROMPT_phase_NN.md` (or legacy flat `PROMPT_<slug>_phase_<N>.md`) under the plan folder (code, tests, lint/typecheck, git). Automated branching and tagging.
...
- **Supporting commands** (defined in `.antigravity/workflows/` and documented in `.antigravity/rules/01-gateflow-ai-workflow.mdc` and `docs/development/guidelines/DEVELOPMENT_WORKFLOWS.md`):
...
- **Plan files** (lifecycle: `docs/development/PLAN_LIFECYCLE.md`; enhancements: `docs/development/plan-guides/PLANNING_ENHANCEMENTS.md`)
  - `Draft/<slug>/` — drafting (`/draft`, early `/plan` scaffolding).
  - `Ready/<slug>/` — approved for execution (via `/plan ready <slug>`).
  - `Active/<slug>/` — `/dev` in flight (moved when a phase starts from Ready).
  - `Complete/<slug>/` — shipped (moved after last phase).
  - `PLAN_<slug>.md`, `TASKS_<slug>.md`, `CONTEXT_<slug>.md`, `PLAN_FEEDBACK.md`, `context/`, `phase_logs/`, `phases/NN_<title>/PROMPT_phase_NN.md` — canonical bundle (see `docs/development/plan-templates/PLAN_FOLDER_STRUCTURE.md`).
  - Archived plans under `Complete/<slug>/` may still use flat `PROMPT_<slug>_phase_N.md`; new plans must use `phases/`.
- **Template & workflow**
  - `docs/development/guidelines/PHASED_DEVELOPMENT_WORKFLOW.md` — end‑to‑end phased workflow (plan → prompts → execute → test → enhance).
  - `docs/development/guidelines/AI_SKILLS_SUBAGENTS_RULES.md` — skills and subagent rules.
  - `docs/development/guidelines/SUBAGENT_HIERARCHY.md` — canonical roles and CLI prefixes.
  - `docs/development/guidelines/DEVELOPMENT_WORKFLOWS.md` — how phased execution integrates with everyday dev.
...
- Use `phases/NN_<title>/PROMPT_phase_NN.md` (or legacy `PROMPT_<slug>_phase_<N>.md`) as the canonical task description for phase N,
...
- **Security skill (Cursor + CLIs)**
  - `.antigravity/skills/gf-security/SKILL.md`
    ...
    - Referenced by the **SECURITY** role in `docs/development/guidelines/SUBAGENT_HIERARCHY.md`.
- **Code contracts**
...
- **Role definitions**
  - `docs/development/guidelines/SUBAGENT_HIERARCHY.md`
...
- **Role personas and orchestrator (Cursor)**
  - `.antigravity/agents/orchestrator.md` — when to invoke subagents, MCP, and multi‑CLI; how to adopt role agents per phase.
  - `.antigravity/agents/roles/` — role‑specific personas (planning, architecture, security, backend‑database, backend‑api, frontend, mobile, QA, i18n, devops, explore).
  - `.antigravity/agents/scenarios/` — scenario prompts (e.g. security‑audit, code‑review).
- **Skills by role (Cursor)**
  - `.antigravity/skills/gf-architecture/SKILL.md` — ARCHITECTURE.
  - `.antigravity/skills/gf-security/SKILL.md` — SECURITY.
  - `.antigravity/skills/gf-database/SKILL.md` — BACKEND‑Database.
  - `.antigravity/skills/gf-api/SKILL.md` — BACKEND‑API.
  - `.antigravity/skills/gf-mobile/SKILL.md` — MOBILE.
  - `.antigravity/skills/gf-testing/SKILL.md` — QA.
  - `.antigravity/skills/gf-i18n/SKILL.md` — i18n.
  - `.antigravity/skills/gf-dev/SKILL.md` — DEVOPS (builds, migrations, preflight).

**Security specialist phases**

- Phases whose **Primary role** is `SECURITY` must:
  - Use `.antigravity/skills/gf-security/SKILL.md` as primary reference.
  - Enforce all invariants from `.antigravity/contracts/CONTRACTS.md` and `.antigravity/rules/00-gateflow-core.mdc`.
  - Treat QR signing, scanner sync (`scanUuid`), multi‑tenant scoping, CSRF, and rate limiting as **non‑negotiable**.
  - Prefer security‑oriented CLIs (Claude) for reviews only when the phase is complex or high‑risk, in line with `docs/development/guidelines/AI_SKILLS_SUBAGENTS_RULES.md` and multi‑CLI guidance.
...
- **Planning & phases**
  - Read from `docs/plan/{Draft,Ready,Active,Complete}/<slug>/PLAN_<slug>.md` and `phases/NN_<title>/PROMPT_phase_NN.md` (older archives may use flat prompts under `Complete/<slug>/`).
...
- **Skills mapping**
  - Mirror the role/skill mapping from `docs/development/guidelines/AI_SKILLS_SUBAGENTS_RULES.md` and `SUBAGENT_HIERARCHY.md` when configuring role‑based behaviors in other IDEs.
...
- **Rule:** `.antigravity/rules/02-gateflow-guide.mdc` (always‑on)
