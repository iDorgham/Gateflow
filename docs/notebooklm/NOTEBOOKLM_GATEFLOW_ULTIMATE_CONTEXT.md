...
> **Last Comprehensive Audit**: August 27, 2026
...
timeline
    ...
    Production Readiness : PWA service worker v2 : Cross-subdomain (.gateflow.site) locale & theme sync : 18 automated CI checks
```
...
- **Phase 6 (PWA Resilience & Cross-Subdomain Sync)**: Upgraded resident portal PWA to a network-first service worker preventing `ERR_FAILED` cache errors, and unified `gf_locale` cookie persistence on `.gateflow.site`.
...
GateFlow's database tier runs on **PostgreSQL** managed by **Prisma 5.x** with 63 models and 41 enums:
...
### 7.1. Atlassian Design System Tokens (`@gateflow/theme`)

- Pure CSS variable tokens (`var(--ds-background-default)`, `var(--ds-text-primary)`, `var(--ds-border-subtle)`).
...
1. **Strict Multi-Tenancy**:
   ...
   - Never write unbounded queries on tenant data.
