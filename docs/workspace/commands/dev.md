...
10. **Save Session Memory + phase log**: Update `SESSION_MEMORY.md` and **`phase_logs/PHASE_LOG_phase_NN.md`**, then finalize session memory (phase + status, last commit hash, decisions, gotchas, files modified, test status, next action or "all phases complete").
