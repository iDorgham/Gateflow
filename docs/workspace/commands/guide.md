...
3. **Reports (Enforced)**
   ...
   - **CLI suggestions** — When a task may be done better in terminal, suggests the right tool from **`docs/guides/TOOL_AND_CLI_REFERENCE.md`**. If the user has listed their tools in **`docs/development/learning/GUIDE_PREFERENCES.md`**, the guide prefers those when they match the task.
...
- **Preferences:** Edit `docs/development/learning/GUIDE_PREFERENCES.md` so the guide adapts to your tone, format, and recurring needs.
...
- When recommending next steps, suggest a CLI when one might do the task better: use **`docs/guides/TOOL_AND_CLI_REFERENCE.md`** and **`docs/development/learning/CLI_TOOL_MEMORY.md`**. Default: Claude = security/architecture; Gemini = schema/DB; Opencode = code gen; Kiro/Qwen = free agentic, large context; Kilo = free terminal, fast iteration.
- If `docs/development/learning/GUIDE_PREFERENCES.md` exists, read it and adapt tone, format, and priorities.
...
- **Learning + memory**:
  - After any work that used a CLI, ensure one entry exists in `docs/development/learning/CLI_USAGE_AND_RESULTS.md`.
  - For durable learnings, point the user to `docs/development/learning/CLI_TOOL_MEMORY.md`.
- Use the skill's output format for the report.
- For super-power mode: respect `docs/development/guidelines/SUBAGENT_HIERARCHY.md`, use phase prompts and acceptance criteria, and run `pnpm preflight`.
