...
- Ensures plan artifacts exist:
  - Resolves plan location per `docs/development/PLAN_LIFECYCLE.md` (`Active/`, `Ready/`, `Draft/`).
  - If no `IDEA_<slug>.md` / `PLAN_<slug>.md` are present, orchestrates `/idea` then `/plan` first.
- For each phase `1..N` in `PLAN_<slug>.md`:
  - Loads `phases/NN_<title>/PROMPT_phase_NN.md` (creating it via `/plan phase <N>` if missing; legacy: `PROMPT_<slug>_phase_<N>.md`).
...
- Respect Claude Pro / Multi-CLI usage limits: follow the phase prompts and `multi-cli-cursor-workflow` skill; only use Multi-CLI for phases that explicitly call for it.
...
- Execute every phase in `PLAN_<slug>.md` (from `Active/`, `Ready/`, or `Draft/`) sequentially.
...
2. **For each phase 1..N**:
   - Equivalent of `/dev <slug> <N>`:
     - `/ready` (preflight).
     - Load `phases/NN_<title>/PROMPT_phase_NN.md` (or legacy flat prompt).
