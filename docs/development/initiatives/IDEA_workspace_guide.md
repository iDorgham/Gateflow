...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Skill**   | Teaches the AI _how_ to be the guide (workspace map, pre-flight, post-task, “what should I do now”, super-power mode, quality checks).                    | **Use.** Single source of truth for guide behavior; reusable from rule and command.                                                                                    |
| **Agent**   | A dedicated “Guide” persona (e.g. in `.cursor/agents/roles/guide.md`).                                                                                    | Optional. The guide is a _mode_ (invoked by rule/command) rather than a separate persona; the skill is enough. Add an agent only if you want a distinct voice/persona. |
