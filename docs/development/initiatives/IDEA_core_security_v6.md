...
- **Scanner rules & policies** (gate–account assignment, location rules, gate hours) are specified in `PRD.md` and `SECURITY_OVERVIEW.md`, but not yet implemented end-to-end across API, DB, and scanner UX.
...
5. **Align implementation with contracts, rules, and docs**
   - Ensure `.cursor/rules/*`, `.cursor/contracts/CONTRACTS.md`, `SECURITY_OVERVIEW.md`, and `PRD.md` all describe the same security reality, backed by tests and audit-logs.
...
- **Docs & contracts alignment**
  - Keep `PRD.md` (security-related sections), `SECURITY_OVERVIEW.md`, `.cursor/rules/*`, and `CONTRACTS.md` synchronized with the implementation as phases ship.
...
- **Contracts and docs are aligned and test-backed**
  ...
  - Security-related sections of `PRD.md` and `SECURITY_OVERVIEW.md` are kept in sync with shipped behavior.
...
2. For each phase, generate `PROMPT_core_security_v6_phase_<N>.md` under `docs/plan/Complete/` that:
   - References `PRD.md`, `SECURITY_OVERVIEW.md`, `.cursor/rules/*`, and `CONTRACTS.md`.
