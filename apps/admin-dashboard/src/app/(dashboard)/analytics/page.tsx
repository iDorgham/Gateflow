...
export default async function AnalyticsPage() {
  ...
  const statusInfo: Record<string, { label: string; color: string; icon: React.ComponentType<{ className?: string }> }> = {
    SUCCESS: { label: 'Success', color: 'text-emerald-600 bg-emerald-500/10', icon: CheckCircle2 },
    DENIED: { label: 'Denied', color: 'text-red-600 bg-red-500/10', icon: XCircle },
    FAILED: { label: 'Failed', color: 'text-amber-600 bg-amber-500/10', icon: AlertCircle },
    EXPIRED: { label: 'Expired', color: 'text-blue-600 bg-blue-500/10', icon: Clock },
    MAX_USES_REACHED: { label: 'Max Uses', color: 'text-violet-600 bg-violet-500/10', icon: AlertCircle },
    INACTIVE: { label: 'Inactive', color: 'text-slate-600 bg-slate-500/10', icon: XCircle },
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">System-wide platform intelligence and usage trends.</p>
      </div>

      {/* KPI summary row */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Organizations', value: totalOrgsCount, sub: 'Active tenants', color: 'text-blue-600', bg: 'bg-blue-500/10', icon: Building2 },
          { label: 'Users', value: totalUsersCount, sub: 'All roles', color: 'text-violet-600', bg: 'bg-violet-500/10', icon: Building2 },
          { label: 'QR Codes', value: totalQRCount, sub: 'Active codes', color: 'text-emerald-600', bg: 'bg-emerald-500/10', icon: QrCode },
          { label: 'Scans (30d)', value: totalScans30, sub: 'Last 30 days', color: 'text-amber-600', bg: 'bg-amber-500/10', icon: BarChart3 },
        ].map((kpi) => (
        ...
        <CardHeader className="border-b border-border pb-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Total scan attempts per day across all organizations.</p>
            </div>
            <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 font-bold text-[10px]">
              {dailyCounts.reduce((a, b) => a + b.count, 0).toLocaleString()} Total
            </Badge>
          </div>
        </CardHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="shadow-md">
          <CardHeader className="border-b border-border pb-4">
            <CardTitle className="text-lg font-bold">Scan Status Breakdown</CardTitle>
            <p className="text-xs text-muted-foreground">Last 30 days, {totalScans30.toLocaleString()} total scans.</p>
          </CardHeader>
                ...
                <div key={key} className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-foreground">{count.toLocaleString()}</span>
        ...
        <Card className="shadow-md">
          <CardHeader className="border-b border-border pb-4">
            <CardTitle className="text-lg font-bold">Top Organizations by Activity</CardTitle>
            <p className="text-xs text-muted-foreground">Scan volume, last 7 days.</p>
          </CardHeader>
          <CardContent className="pt-5 space-y-3">
            {topOrgs.length === 0 ? (
              <p className="text-sm text-muted-foreground italic">No scan activity in the last 7 days.</p>
            ) : (
              topOrgs.map((org, i) => {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="shadow-md">
          <CardHeader className="border-b border-border pb-4">
            ...
            <p className="text-xs text-muted-foreground">Active organizations by subscription tier.</p>
          </CardHeader>
              ...
              return (
                <div key={plan} className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-foreground">{count.toLocaleString()} orgs</span>
        ...
        <Card className="shadow-md">
          <CardHeader className="border-b border-border pb-4">
            <CardTitle className="text-lg font-bold">QR Code Types</CardTitle>
            <p className="text-xs text-muted-foreground">Active QR codes by type across all organizations.</p>
          </CardHeader>
          <CardContent className="pt-5 space-y-4">
            {[
              { type: 'SINGLE', color: 'bg-emerald-500', desc: 'One-time use' },
              { type: 'RECURRING', color: 'bg-blue-500', desc: 'Repeating access' },
              { type: 'PERMANENT', color: 'bg-violet-500', desc: 'No expiry' },
            ].map(({ type, color, desc }) => {
