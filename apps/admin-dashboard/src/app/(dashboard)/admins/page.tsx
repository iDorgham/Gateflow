...
export default async function AdminsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Admins</h1>
        <p className="text-muted-foreground mt-1">
          Manage super-admin users who have platform-level access.
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-blue-900 dark:text-blue-300">Two-layer Admin Auth</p>
              <p className="text-xs text-blue-700 dark:text-blue-400">
                This admin dashboard is protected by <code className="rounded bg-blue-100 dark:bg-blue-900/50 px-1 font-mono">ADMIN_ACCESS_KEY</code> (env var).
                ADMIN-role users below are platform super-admins who can access the client dashboard with elevated privileges.
                They are different from the dashboard login key.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <Card className="border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 shadow-lg shadow-amber-500/10">
          ...
          <CardContent className="pt-4 space-y-3">
            ...
            <div className="relative group">
              <code className="block w-full rounded-xl bg-amber-100/80 dark:bg-amber-900/40 border border-amber-200 dark:border-amber-700 px-6 py-4 font-mono text-xl font-black tracking-[0.2em] text-amber-950 dark:text-amber-200 text-center shadow-inner">
                {pwFlash.pw}
              </code>
          ...
          {admins.length === 0 ? (
            <Card className="shadow-sm">
              <CardContent className="py-10 text-center text-muted-foreground">
                ...
                <p className="text-xs mt-1">Create the first admin using the form.</p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <form action={toggleSuspend}>
                            ...
                            >
                              {suspended ? (
                                <><ShieldCheck className="h-3 w-3 mr-1.5" />Restore</>
                              ) : (
                                <><ShieldAlert className="h-3 w-3 mr-1.5" />Suspend</>
                              )}
                            </Button>
                          </form>
                        </div>
