...
import { Database, Building2, ArrowRight } from 'lucide-react';
import Link from 'next/link';

export const metadata = { title: 'Advanced Seeding | Global' };

export default async function GlobalSeedingPage(props: {
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 space-y-8 animate-in fade-in duration-700">
      <div className="relative">
        ...
        <div className="relative bg-ds-background-default border-2 border-ds-border p-8 rounded-[2.5rem] shadow-xl">
          <Database className="h-20 w-20 text-ds-text-brand" />
        </div>
      </div>
      ...
      <div className="max-w-md space-y-4">
        <PageHeader
          ...
          subtitle="Database seeding and unit initialization requires an active organization context to target."
          className="text-center"
        />
        <p className="text-sm text-ds-text-subtlest font-medium uppercase tracking-widest">
          Select an organization to initialize seeding operations.
        </p>
      </div>
