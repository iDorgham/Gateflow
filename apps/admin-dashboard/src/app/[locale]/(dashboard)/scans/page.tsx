...
import { ScanLine, Building2, ArrowRight } from 'lucide-react';
import Link from 'next/link';

export const metadata = { title: 'Scan Traffic | Global' };

export default async function GlobalScansPage(props: {
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 space-y-8 animate-in fade-in duration-700">
      <div className="relative">
        ...
        <div className="relative bg-ds-background-default border-2 border-ds-border p-8 rounded-[2.5rem] shadow-xl">
          <ScanLine className="h-20 w-20 text-ds-text-brand" />
        </div>
      </div>
      ...
      <div className="max-w-md space-y-4">
        <PageHeader
          title="Scan Traffic"
          subtitle="Real-time scan logs and access history are scoped to individual organizations for security and isolation."
          className="text-center"
        />
        <p className="text-sm text-ds-text-subtlest font-medium uppercase tracking-widest">
          Select an organization to view its real-time traffic and access logs.
        </p>
      </div>
