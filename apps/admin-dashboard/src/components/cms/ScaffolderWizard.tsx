...
export function ScaffolderWizard({ orgId, orgName, locale }: ScaffolderWizardProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="text-center space-y-4 mb-12">
        ...
        <h1 className="text-4xl font-black uppercase tracking-tight italic">AI Page Architect</h1>
        <p className="text-ds-text-subtle max-w-lg mx-auto">
          Describe your objective, and our AI will architect a high-conversion landing page structure and draft persuasive copy for you.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-center gap-4 mb-12">
          <div className={cn("w-3 h-3 rounded-full transition-all duration-500", step === 1 ? "bg-blue-600 scale-125 shadow-lg shadow-blue-500/40" : "bg-ds-border")}></div>
          <div className="w-12 h-0.5 bg-ds-border"></div>
          <div className={cn("w-3 h-3 rounded-full transition-all duration-500", step === 2 ? "bg-blue-600 scale-125 shadow-lg shadow-blue-500/40" : "bg-ds-border")}></div>
        </div>
        ...
        {step === 1 ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                    <p className="text-[11px] text-amber-700/80 italic leading-relaxed">
                      AI will automatically generate high-converting copy in both **English** and **Arabic**, optimized for MENA market psychology.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6 animate-in zoom-in-95 duration-500">
            <div className="flex items-center justify-between mb-2 px-2">
              <h3 className="font-black uppercase tracking-tight text-xl">Architectural Blueprint Ready</h3>
              <div className="flex gap-3">
                <Button variant="outline" className="rounded-xl h-10 border-ds-border/40" onClick={() => setStep(1)}>
                  Back
                </Button>
                <Button className="bg-blue-600 hover:bg-blue-700 rounded-xl h-10 shadow-lg shadow-blue-500/20 gap-2" onClick={handleSave} disabled={isGenerating}>
              ...
              <CardContent className="p-8 space-y-8">
                <div className="flex items-center gap-6 pb-6 border-b border-ds-border/40">
                  ...
                  <div className="flex-1">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-ds-text-subtle mb-1">Page Title & Slug</h4>
                    <div className="flex items-center gap-4">
                      <div className="text-xl font-bold">{result?.titleEn}</div>
                      <Badge variant="outline" className="font-mono text-[10px]">/{result?.slug}</Badge>
                    </div>
                  </div>
                </div>

                <div className="space-y-6">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-ds-text-subtle">Structure & Sections</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {result?.sections?.map((section: any, idx: number) => (
                      <div key={idx} className="p-4 rounded-2xl border border-ds-border/40 bg-slate-50/50 flex items-center gap-4 hover:border-blue-500/20 hover:bg-blue-50/30 transition-all">
                        ...
                        <div className="flex-1">
                          <div className="text-[10px] font-black uppercase tracking-widest text-blue-600">{section.type}</div>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 rounded-2xl bg-slate-900 text-white">
                   <div className="space-y-1">
                     ...
                     <div className="text-[11px] text-slate-300 italic">Optimized for clarity and global reach.</div>
                   </div>
                   <div className="space-y-1">
                     <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-slate-400">
                       <Globe className="w-3 h-3" /> Arabic Content
                     </div>
                     <div className="text-[11px] text-slate-300 italic text-right" dir="rtl">مُحسّن للثقافة المحلية وسيكولوجية السوق.</div>
                   </div>
                </div>
              </CardContent>
            </Card>
