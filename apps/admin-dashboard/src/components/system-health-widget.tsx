...
import { Card, CardHeader, CardTitle, CardContent, cn } from '@gateflow/ui';
  ...
  return (
    <Card className={cn('overflow-hidden', className)}>
      ...
      >
        ...
        <div
          className={cn(
            'px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize flex items-center gap-1.5',
        ...
        >
          <span
            className={cn(
              ...
              overallStatus !== 'down' && 'animate-pulse'
            )}
          />
          {t(
            `admin.overview.componentStatus${overallStatus.charAt(0).toUpperCase() + overallStatus.slice(1)}`
          )}
        </div>
      </CardHeader>
      <CardContent className="p-0">
        <div className="divide-y">
          {services.map((svc) => (
            <div
              key={svc.name}
              className="flex px-4 py-3 items-center justify-between hover:bg-muted/50 transition-colors"
            >
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                >
                  ...
                  {t(
                    `admin.overview.componentStatus${svc.status.charAt(0).toUpperCase() + svc.status.slice(1)}`
                  )}
                </span>
                <span
                  className="text-xs text-muted-foreground font-mono bg-muted px-1.5 rounded"
                  dir="ltr"
                >
                  {svc.latencyMs}ms
                </span>
