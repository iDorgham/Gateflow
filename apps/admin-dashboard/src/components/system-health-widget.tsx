...
import { Card, CardHeader, CardTitle, CardContent, cn } from "@gate-access/ui";
    ...
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader className={cn("pb-4 flex flex-row items-center justify-between", statusBg[overallStatus])}>
        ...
        <div className={cn("px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize flex items-center gap-1.5", statusColor[overallStatus])}>
          ...
          {t(`admin.overview.componentStatus${overallStatus.charAt(0).toUpperCase() + overallStatus.slice(1)}`)}
        </div>
      </CardHeader>
            ...
            <div key={svc.name} className="flex px-4 py-3 items-center justify-between hover:bg-muted/50 transition-colors">
              ...
              <div className="flex flex-col items-end gap-1">
                <span className={cn(
                  ...
                  {t(`admin.overview.componentStatus${svc.status.charAt(0).toUpperCase() + svc.status.slice(1)}`)}
                </span>
                <span className="text-xs text-muted-foreground font-mono bg-muted px-1.5 rounded" dir="ltr">
                  {svc.latencyMs}ms
                </span>
              </div>
            </div>
