...
export function StyleHubClient({ orgId, initialVariables, snapshots }: StyleHubProps) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ds-text-subtle">
            Customize your organization's visual identity with AI-driven design tokens.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <Card className="border-ds-border/40 shadow-sm overflow-hidden">
            ...
            <CardContent className="pt-6 space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-wider text-ds-text-subtle">Describe your vision</label>
                <textarea 
                  value={prompt}
          ...
          <Card className="border-ds-border/40 shadow-sm">
            <CardHeader>
              <CardTitle className="text-xs font-black uppercase tracking-widest">Manual Token Override</CardTitle>
            </CardHeader>
                  ...
                  return (
                    <div key={token} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <code className="text-[9px] font-mono text-ds-text-subtle">{token}</code>
                        {token.includes('color') || token.includes('background') || token.includes('surface') ? (
                          <div className="w-4 h-4 rounded-full border border-ds-border shadow-sm" style={{ backgroundColor: currentValue }} />
                        ) : null}
                      </div>
                      <Input 
                        value={currentValue}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="flex items-center justify-between bg-ds-background-subtle/50 p-1.5 rounded-2xl border border-ds-border/40 mb-6">
              <TabsList className="bg-transparent border-none gap-2">
                <TabsTrigger value="editor" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm px-6 h-9 text-[10px] font-black uppercase tracking-widest">
                  <Layout className="w-4 h-4 mr-2" />
                  Live Preview
                </TabsTrigger>
                <TabsTrigger value="history" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm px-6 h-9 text-[10px] font-black uppercase tracking-widest">
            ...
            <TabsContent value="editor" className="m-0 focus-visible:outline-none">
              <div className="rounded-2xl border-4 border-ds-border/20 shadow-2xl overflow-hidden aspect-video bg-white relative group">
                <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button size="sm" variant="secondary" className="rounded-full shadow-lg h-8 gap-2">
                ...
                <div className="w-full h-full p-8 space-y-8 overflow-y-auto">
                   <div className="flex items-center justify-between">
                     <div className="w-32 h-8 rounded-lg" style={{ backgroundColor: 'var(--ds-background-brand-bold)' }} />
                     <div className="flex gap-4">
                       <div className="w-20 h-8 rounded-full" style={{ backgroundColor: 'var(--ds-background-subtle)' }} />
                       <div className="w-8 h-8 rounded-full" style={{ backgroundColor: 'var(--ds-background-neutral-subtle)' }} />
                     </div>
                   </div>

                   <div className="grid grid-cols-3 gap-6">
                     {[1, 2, 3].map(i => (
                       <Card key={i} className="border-ds-border/40 overflow-hidden" style={{ borderRadius: 'var(--ds-radius-default)' }}>
                         <div className="h-24" style={{ backgroundColor: 'var(--ds-background-brand-subtle)' }} />
                         <CardContent className="pt-4 space-y-2">
                           <div className="h-3 w-3/4 rounded" style={{ backgroundColor: 'var(--ds-text)' }} />
                           <div className="h-2 w-1/2 rounded" style={{ backgroundColor: 'var(--ds-text-subtle)' }} />
                         </CardContent>
                       </Card>
                     ))}
                   </div>
                   ...
                   <div className="flex gap-4">
                     <Button className="font-bold italic" style={{ backgroundColor: 'var(--ds-background-brand-bold)', borderRadius: 'var(--ds-radius-default)' }}>
                       PRIMARY ACTION
                     </Button>
                     <Button variant="outline" className="border-ds-border" style={{ borderRadius: 'var(--ds-radius-default)' }}>
                       Secondary Action
                     </Button>
                   </div>
                </div>
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="p-4 rounded-xl border border-ds-border-success/30 bg-ds-background-success-subtle/5 flex items-center gap-4">
                   ...
                   <div>
                     <h4 className="text-[10px] font-black uppercase tracking-widest text-ds-text-success">WCAG AA Compliant</h4>
                     <p className="text-[11px] text-ds-text-subtle">Contrast ratio for brand primary is 5.2:1</p>
                   </div>
                 </div>
                 <div className="p-4 rounded-xl border border-ds-border/40 bg-ds-background-subtle/5 flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-[10px] font-black uppercase tracking-widest">Layout Integrity</h4>
                      <p className="text-[11px] text-ds-text-subtle">Border radius and spacing variables verified.</p>
                    </div>
                 </div>
              </div>
            </TabsContent>
                  ...
                  <div className="divide-y divide-ds-border/40">
                    {snapshots.length === 0 ? (
                      ...
                      snapshots.map((snapshot) => (
                        <div key={snapshot.id} className="p-6 flex items-center justify-between hover:bg-ds-background-neutral-subtle/30 transition-colors">
                          <div className="flex items-center gap-4">
                            <div className="p-3 rounded-xl bg-purple-500/10 text-purple-600">
                              <History className="w-5 h-5" />
                            </div>
                            <div>
                              <div className="text-sm font-bold">{snapshot.name}</div>
                              <div className="text-[10px] text-ds-text-subtle uppercase tracking-wider">
                                {new Date(snapshot.createdAt).toLocaleString()} • {Object.keys(snapshot.cssTokens).length} Tokens
                              </div>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" className="h-8 text-[9px] font-black uppercase tracking-widest px-4 border-ds-border/40">
                              <Eye className="w-3.5 h-3.5 mr-2" />
                              View
                            </Button>
                            <Button size="sm" variant="outline" className="h-8 text-[9px] font-black uppercase tracking-widest px-4 border-ds-border/40 hover:bg-purple-500 hover:text-white hover:border-purple-500">
