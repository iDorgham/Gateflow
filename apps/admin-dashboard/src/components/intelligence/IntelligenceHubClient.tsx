...
export function IntelligenceHubClient({ orgId, initialSources }: IntelligenceHubProps) {
  ...
  const handleSync = async (sourceId: string) => {
    ...
    try {
      ...
      setSources(prev => prev.map(s => s.id === sourceId ? { ...s, status: 'SYNCED', lastSyncedAt: new Date().toISOString() } : s));
    } finally {
      setIsSyncing(null);
    }
  };
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="border-ds-border/40 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Badge variant="outline" className="text-[9px] font-black">{sources.length} Indexed</Badge>
            </CardHeader>
            <CardContent className="pt-4">
              <div className="space-y-4">
                {sources.map((source) => (
                  <div key={source.id} className="p-4 rounded-xl border border-ds-border/40 bg-slate-50/30 flex items-center justify-between hover:border-indigo-500/20 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="p-2.5 rounded-lg bg-white shadow-sm border border-ds-border/40">
                        {source.type === 'WEBSITE' ? <Globe className="w-4 h-4 text-blue-500" /> : <Upload className="w-4 h-4 text-indigo-500" />}
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Badge 
                            className={cn(
                              ...
                              source.status === 'SYNCED' ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                            )}
                          >
                            {source.status}
                          </Badge>
                          <span className="text-[10px] text-ds-text-subtle flex items-center gap-1">
                            ...
                            {source.lastSyncedAt ? new Date(source.lastSyncedAt).toLocaleDateString() : 'Never'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <RefreshCw className={cn("w-4 h-4", isSyncing === source.id && "animate-spin")} />
                    </Button>
                  </div>
                ))}

                <Button variant="outline" className="w-full border-dashed border-2 py-8 rounded-2xl gap-2 hover:bg-slate-50">
          ...
          <Card className="border-ds-border/40 bg-indigo-600 text-white shadow-xl shadow-indigo-500/10">
            <CardContent className="p-6">
               <div className="flex items-center gap-3 mb-4">
                 ...
                 <h4 className="font-black uppercase tracking-tight text-sm italic">Intelligence Core v1.0</h4>
               </div>
               <p className="text-xs text-indigo-100 leading-relaxed">
                 All knowledge is stored as high-dimensional vector embeddings, enabling semantic search that understands the **context** of your operational rules, not just keywords.
               </p>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="border-ds-border/40 shadow-xl overflow-hidden flex flex-col flex-1 min-h-[600px] bg-white">
            <CardHeader className="bg-slate-50 border-b border-ds-border/40 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-sm font-black uppercase tracking-tight italic">Vertical Assistant</CardTitle>
                    <div className="flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
                      <span className="text-[10px] text-ds-text-subtle font-bold uppercase tracking-widest">Grounding Active</span>
                    </div>
                  </div>
                </div>
                <Button variant="ghost" size="sm" className="h-8 text-[10px] font-black uppercase tracking-widest text-ds-text-subtle">
                  Clear History
                </Button>
              </div>
            </CardHeader>
                  ...
                  chatHistory.map((msg, i) => (
                    <div key={i} className={cn("flex gap-4", msg.role === 'user' ? "flex-row-reverse" : "")}>
                      <div className={cn(
                        ...
                        {msg.role === 'user' ? <Search className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                      </div>
                      <div className={cn(
                        "p-4 rounded-2xl max-w-[85%] text-sm leading-relaxed",
                        msg.role === 'user' ? "bg-slate-900 text-white rounded-tr-none" : "bg-slate-100 text-slate-800 rounded-tl-none border border-ds-border/40"
                      )}>
                        {msg.content}
                      </div>
                    </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-ds-border/40">
               ...
               <div className="mt-3 flex items-center justify-center gap-4">
                 <span className="text-[10px] text-ds-text-subtle font-black uppercase tracking-widest">Suggested Queries:</span>
                 <button onClick={() => setQuery("Visitor access policy")} className="text-[10px] text-indigo-600 hover:underline font-bold underline-offset-2">Visitor Policy</button>
                 <button onClick={() => setQuery("Emergency contacts")} className="text-[10px] text-indigo-600 hover:underline font-bold underline-offset-2">Emergency Hub</button>
               </div>
            </div>
          </Card>
        </div>
