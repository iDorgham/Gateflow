...
export function TaskManagerClient({
  ...
  const activeBoard =
    initialBoards.find((b) => b.id === activeBoardId) || initialBoards[0];
  const tasks = activeBoard?.tasks || [];
  const selectedTask = tasks.find((t: any) => t.id === selectedTaskId) || null;

  const handleGenerateTasks = async () => {
    const prompt = window.prompt(
      'Enter a project description for AI task generation:'
    );
    if (!prompt) return;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage departmental workflows and AI automation.
          </p>
        </div>
      ...
      <div className="flex gap-4 items-center bg-white p-2 rounded-xl border shadow-sm">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-auto">
          <TabsList className="bg-transparent gap-1">
            <TabsTrigger
              value="kanban"
              className="data-[state=active]:bg-gray-100 gap-2"
            >
                ...
                >
                  <div className="flex justify-between items-center px-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {
                          tasks.filter((t: any) => t.status === status.id)
                            .length
                        }
                      </Badge>
                    </div>
                    ...
                    {tasks.map((task: any) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <Badge
                            variant="outline"
                            className="capitalize gap-1.5 border-blue-100 bg-blue-50 text-blue-700"
                          >
            ...
            >
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  MENA Region Calendar
                </h3>
                <p className="text-gray-500 mt-1 max-w-md">
                  Optimized for Fri-Sat weekend schedules. This view is
                  currently under construction.
                </p>
              </div>
              <Button variant="outline" onClick={() => setActiveTab('kanban')}>
                Back to Kanban
              </Button>
            </motion.div>
          )}
      ...
      <CardContent className="p-3.5 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {task.linkedType && (
            <Badge
              variant="outline"
              className="text-[10px] uppercase h-5 bg-gray-50"
            >
...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn('text-[10px] h-5', styles[priority])}
    >
      {priority}
    </Badge>
  );
