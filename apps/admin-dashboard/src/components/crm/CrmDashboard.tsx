...
import { Target, MessageSquare, TrendingUp, Sparkles, Search, Filter, Eye, ChevronRight, GripVertical } from 'lucide-react';
...
export function CrmDashboard({ leads: initialLeads, locale, translations }: CrmDashboardProps) {
  ...
  const handleScoreLead = async (leadId: string) => {
    ...
    try {
      ...
      setLeads(prev => prev.map(l => 
        l.id === leadId ? { ...l, score: result.score } : l
      ));
  ...
  const handleGenerateDraft = async (leadId: string) => {
    ...
    try {
      const res = await fetch('/api/crm/generate-draft', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ leadId, tone: 'professional', language: locale.split('-')[0] }),
      });
    ...
    <div className="flex flex-col h-full gap-6 animate-in fade-in duration-500 overflow-hidden">
      <div className="shrink-0 space-y-6">
        <PageHeader
          ...
          icon={<div className="p-2 rounded-xl bg-ds-background-brand-bold text-white shadow-lg shadow-brand/20"><Target className="h-5 w-5" /></div>}
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-ds-background-neutral-subtle/40 border-none shadow-none ring-1 ring-ds-border/50">
            ...
            <CardContent>
              <div className="text-3xl font-black tabular-nums">{leads.length}</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-ds-background-brand-subtle/20 border-none shadow-none ring-1 ring-ds-border-brand/30">
            ...
            <CardContent>
              <div className="text-3xl font-black text-ds-text-brand tabular-nums">{highIntentCount}</div>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        <div className="flex h-full gap-4 min-w-max items-start">
          {KANBAN_STAGES.map(stage => {
            ...
            return (
              <div key={stage.id} className="flex flex-col w-72 h-full max-h-full bg-ds-background-neutral-subtle/30 rounded-xl border border-ds-border/50">
                <div className="p-3 border-b border-ds-border/50 flex justify-between items-center shrink-0">
                  <h3 className="text-xs font-black uppercase tracking-wider text-ds-text-subtle">{stage.label}</h3>
                  <Badge variant="outline" className="text-[10px] tabular-nums font-bold bg-ds-background-default">
                    {stageLeads.length}
                  </Badge>
                </div>
                <div className="p-2 overflow-y-auto flex-1 space-y-2">
                  {stageLeads.map(lead => {
                    ...
                    const colorClass = score && score >= 80 ? 'text-green-500' : score && score >= 50 ? 'text-yellow-500' : score !== null ? 'text-red-500' : 'text-ds-text-subtle';
                    return (
                      <Card 
                        key={lead.id} 
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex flex-col min-w-0 pr-2">
                            <span className="font-bold text-sm text-ds-text truncate">
                              {lead.firstName || lead.lastName ? `${lead.firstName || ''} ${lead.lastName || ''}`.trim() : lead.organizationName}
                            </span>
                            {(lead.firstName || lead.lastName) && lead.company && (
                              <span className="text-[10px] text-ds-text-subtle truncate">{lead.company}</span>
                            )}
                        ...
                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-ds-border/30">
                          <div className="flex items-center gap-1.5" title="AI Score">
                            {score !== null ? (
                              <>
                                <div className={cn("h-1.5 w-1.5 rounded-full", score >= 80 ? 'bg-green-500' : score >= 50 ? 'bg-yellow-500' : 'bg-red-500')} />
                                <span className={cn("font-black text-xs tabular-nums", colorClass)}>{score}</span>
                              </>
                            ) : (
                              <span className="text-[9px] font-bold tracking-widest uppercase text-ds-text-subtlest">UNRATED</span>
                            )}
                          </div>
                  ...
                  {stageLeads.length === 0 && (
                    <div className="h-24 flex items-center justify-center text-center p-4 border-2 border-dashed border-ds-border/50 rounded-lg">
                      <span className="text-[10px] font-bold text-ds-text-subtlest uppercase tracking-widest">No Leads</span>
                    </div>
                  )}
                </div>
              </div>
