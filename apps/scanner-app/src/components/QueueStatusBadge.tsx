...
export function QueueStatusBadge({ queueSize }: QueueStatusBadgeProps) {
  ...
  const getColors = () => {
    if (isCriticalVolume)
      return {
        bg: `${nativeTokens.colors.danger}20`,
        text: nativeTokens.colors.danger,
        icon: AlertCircle,
      };
    if (isHighVolume)
      return {
        bg: `${nativeTokens.colors.warning}20`,
        text: nativeTokens.colors.warning,
        icon: Users,
      };
    return {
      bg: `${nativeTokens.colors.primary}20`,
      text: nativeTokens.colors.primary,
      icon: Users,
    };
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: bg }]}>
      ...
      <Text style={[styles.text, { color: text }]}>{queueSize} in queue</Text>
    </View>
  );
}
