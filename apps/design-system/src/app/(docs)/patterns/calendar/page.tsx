...
const calendarPrinciples = [
  {
    ...
    description:
      'Active date selections use the accent profile glow, while background cells use the Satin-Charcoal hierarchy for contrast.',
    icon: Calendar,
  },
  {
    title: 'Institutional Availability',
    description:
      'Available slots use --gf-color-success (Emerald), while restricted or blocked slots use --ds-border-bold with low opacity.',
    icon: Lock,
  },
  {
    title: 'Time-Flow Transitions',
    description:
      'Changing months or clicking through time slots uses horizontal sliding animations (0.3s) to maintain temporal continuity.',
    icon: Clock,
  },
];
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm font-bold text-[var(--ds-text-subtle)] opacity-60">
            Interactive demonstration of date selection, range picking, and
            availability logic.
          </p>
        </div>
        <ScheduleLab />
        ...
        <div className="flex gap-4 relative z-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-[var(--ds-text-subtle)] leading-relaxed max-w-2xl font-medium">
              In GateFlow, time is a high-stakes resource. Calendars must be
              dense enough to show full schedules but clear enough to prevent
              booking errors. We leverage <strong>--ds-border-bold</strong> for
              day cells and <strong>--ds-text-accent</strong> for today&apos;s
              date indicator.
            </p>
          </div>
        </div>
    ...
    <div className="w-full max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="p-8 rounded-3xl border border-[var(--ds-border-bold)] bg-[var(--ds-background-default)] shadow-2xl flex flex-col gap-8">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[10px] font-bold text-[var(--ds-text-subtle)] uppercase tracking-widest">
          ...
          <div className="flex gap-1">
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 rounded-lg bg-[var(--ds-surface-subtle)] border border-white/5"
            >
              <ChevronLeft size={16} />
            </Button>
            <Button
        ...
        <div className="grid grid-cols-7 gap-2">
          {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d) => (
            <div
              key={d}
              className="h-8 flex items-center justify-center text-[9px] font-black text-[var(--ds-text-subtlest)]"
            >
              {d}
            </div>
          ))}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-[var(--ds-background-brand-bold)] text-white shadow-[var(--ds-primary-accent)]/10 ring-2 ring-[var(--ds-primary-accent)]/20'
                    : isAvailable
                      ? 'hover:bg-[var(--ds-surface-subtle)] text-[var(--ds-text-primary)] border border-white/5'
      ...
      <div className="p-8 rounded-3xl border border-[var(--ds-border-bold)] bg-[var(--ds-surface-subtle)] flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          <Badge
            variant="outline"
            className="w-fit bg-[var(--ds-background-brand-bold)]/10 text-[var(--ds-text-brand)] border-[var(--ds-border-brand)]/20 text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full"
          >
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-[var(--ds-background-default)] border-[var(--ds-primary-accent)] shadow-xl'
                      : 'bg-[var(--ds-surface-subtle)] border-white/5 hover:border-white/10'
                  )}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-bold text-[var(--ds-text-subtlest)] uppercase tracking-tight">
                        Main Gate Protocol
                      </span>
                    </div>
                  </div>
                  {isActive && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                      <CheckCircle2
                        size={16}
                        className="text-[var(--gf-color-success)]"
                      />
                    </motion.div>
                  )}
                </div>
