...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, PieChart } from 'recharts';
    ...
    <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 p-8 rounded-3xl border border-[var(--ds-border-bold)] bg-[var(--ds-background-default)] shadow-2xl relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none" 
             style={{ backgroundImage: 'radial-gradient(var(--ds-text-primary) 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <XAxis 
                ...
                axisLine={false} 
                tickLine={false} 
                tick={{ fill: 'var(--ds-text-subtlest)', fontSize: 10, fontWeight: 800 } as any}
                dy={10}
              />
              <YAxis 
                axisLine={false} 
      ...
      <div className="flex flex-col gap-4">
            ...
            {chartData.map(item => (
              <div key={item.name} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-[var(--ds-text-primary)]">{item.name} Series</span>
                </div>
                <code className="text-[9px] font-mono text-[var(--ds-text-subtlest)]">{item.color}</code>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl border border-[var(--ds-accent-bold)]/20 bg-[var(--ds-accent-subtle)]/10 flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[var(--ds-accent-bold)]">
            <PieChart size={14} />
            <span className="text-[10px] font-black uppercase tracking-widest">Profile Sync</span>
          </div>
          <p className="text-[10px] font-medium text-[var(--ds-text-subtle)] leading-relaxed">
            The first series of any chart is bound to <strong>--ds-primary-accent</strong>. Try changing your accent profile in the Lab to see this dashboard update instantly.
          </p>
        </div>
      </div>
    </div>
