...
import { ShieldCheck, Palette, LayoutTemplate, Info, Laptop, Monitor, Mail, Lock, ChevronRight, Globe } from 'lucide-react';
...
const authPrinciples = [
  {
    ...
    description: 'Auth pages are primary brand touchpoints. They automatically inherit the Tenant Profile (Kimchi/Cobalt/Emerald) to provide instant visual verification.',
    icon: Palette,
  },
  {
    title: 'Cine-Entrance Protocol',
    description: 'Auth forms use staggered sequence animation (0.05s delay per field) to create a premium, orchestrated experience for users.',
    icon: LayoutTemplate,
  },
  {
    title: 'Visual Security Proof',
    description: 'Authentication succeeds with the "Institutional Green" bloom effect, providing a positive physiological feedback loop for secure access.',
    icon: ShieldCheck,
  },
];
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm font-bold text-[var(--ds-text-subtle)] opacity-60">
            Switch the tenant profile to see how the login experience adapts its institutional accent.
          </p>
        </div>
        ...
        <div className="flex gap-4 relative z-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-[var(--ds-text-subtle)] leading-relaxed max-w-2xl font-medium">
              Regardless of the tenant accent, auth pages always maintain the <strong>Satin-Charcoal</strong> base (--gf-color-neutral-10). This creates a consistent &quot;GateFlow Guardian&quot; feel across the entire monorepo, reinforcing professional security standards.
            </p>
          </div>
        </div>
      </section>
...
function AuthProfileLab({ profile, onProfileChange }: { profile: string, onProfileChange: (p: string) => void }) {
  const getProfileColors = (p: string) => {
    switch(p) {
        case 'kimchi': return { accent: 'oklch(62% 0.22 35)', label: 'Kimchi Energy' };
        case 'cobalt': return { accent: 'oklch(50% 0.18 250)', label: 'Cobalt Finance' };
        case 'emerald': return { accent: 'oklch(65% 0.15 155)', label: 'Emerald Health' };
        default: return { accent: 'oklch(62% 0.22 35)', label: 'Kimchi Energy' };
    }
  };
    ...
    <div className="flex flex-col gap-8 w-full">
        ...
        {['kimchi', 'cobalt', 'emerald'].map(p => (
           <Button
            ...
            className={cn(
                ...
                profile === p ? "bg-[var(--ds-background-selected)] text-[var(--ds-text-selected)] border-[var(--ds-border-brand)] ring-1" : "bg-transparent border-[var(--ds-border-subtle)]"
            )}
           >
             <div className="h-2 w-2 rounded-full" style={{ backgroundColor: getProfileColors(p).accent }} />
      ...
      <div className="w-full max-w-4xl mx-auto rounded-3xl border border-[var(--ds-border-bold)] bg-[var(--ds-background-default)] shadow-2xl p-1 relative overflow-hidden h-[600px] flex group">
        ...
        <div className="flex-1 rounded-[22px] overflow-hidden bg-background flex flex-col shadow-inner border border-white/5">
             ...
             {/* Auth Content Mock */}
             <div className="flex-1 flex" style={{ '--current-accent': colors.accent } as any}>
                {/* Visual Left Side */}
                <div className="hidden lg:flex flex-1 relative overflow-hidden bg-[var(--ds-surface-subtle)] border-e border-black/20 p-12 flex-col justify-between">
                    <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: 'var(--gf-pattern-sentinel)', backgroundSize: '40px 40px' }} />
                    ...
                    <div className="flex items-center gap-2 group cursor-pointer relative z-10">
                        ...
                        <span className="text-2xl font-black tracking-tight text-white">GateFlow</span>
                    </div>
                    ...
                    <div className="relative z-10 flex flex-col gap-4">
                        ...
                        <h2 className="text-3xl font-black tracking-tighter text-white leading-none">Institutional Security,<br/>Refined.</h2>
                        <p className="text-sm font-medium text-[var(--ds-text-subtle)] max-w-sm">Secure access for enterprise tenants utilizing advanced OKLCH biometric logic.</p>
                    </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[var(--ds-background-default)]">
                    <div className="w-full max-w-xs flex flex-col gap-8">
                        <div className="flex flex-col gap-2">
                             <h3 className="text-xl font-black uppercase tracking-tight text-white">Welcome Back</h3>
                             <p className="text-[10px] font-bold text-[var(--ds-text-subtle)] uppercase tracking-widest">Sign in to your secure workspace</p>
                        </div>

                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col gap-2">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtlest)]">Email Address</Label>
                                <div className="relative">
                                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--ds-text-subtlest)]" />
                                    <Input defaultValue="admin@tenant.corp" className="bg-[var(--ds-surface-subtle)] border-white/5 pl-10 h-12 focus-visible:ring-[var(--current-accent)] text-sm" />
                                </div>
                            </div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtlest)]">Security Key</Label>
                                <div className="relative">
                                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--ds-text-subtlest)]" />
                                    <Input type="password" defaultValue="********" className="bg-[var(--ds-surface-subtle)] border-white/5 pl-10 h-12 focus-visible:ring-[var(--current-accent)]" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-center gap-2 pt-4 border-t border-white/5">
                            <span className="text-[9px] font-black uppercase tracking-widest text-[var(--ds-text-subtlest)]">Advanced Guardian Protection Active</span>
                        </div>
                    </div>
                </div>
             </div>
