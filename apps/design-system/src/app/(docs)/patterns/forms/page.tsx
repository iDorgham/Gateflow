...
import { ClipboardCheck, ShieldAlert, Zap, Info, ChevronRight, CheckCircle2, LayoutPanelLeft } from 'lucide-react';
...
const formPrinciples = [
  {
    ...
    description: 'Input focus should NOT just use a border. We use a 2px offset ring combined with a subtle outer glow bound to the primary accent.',
    icon: Zap,
  },
  {
    title: 'Institutional Gold',
    description: 'Validation warnings use --gf-color-warning (Institutional Gold). It projects seriousness without the panic of standard red.',
    icon: ShieldAlert,
  },
  {
    title: 'Cine-Stagger Steps',
    description: 'Multi-step forms use horizontal motion transitions (easeInOut) to provide context and reduce cognitive load during long flows.',
    ...
    <div className="flex flex-col gap-12 max-w-5xl mx-auto py-10 px-6">
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm font-bold text-[var(--ds-text-subtle)] opacity-60">
            Interactive demonstration of tiered form transitions and validation states.
          </p>
        </div>
        <FormStepLab />
      </div>
      ...
      <section className="p-8 rounded-3xl border border-[var(--ds-border-bold)] bg-[var(--ds-surface-subtle)] relative overflow-hidden group">
        <div className="absolute top-2 right-2 p-2 opacity-10 scale-150 rotate-12 group-hover:rotate-0 transition-all duration-700">
          <ClipboardCheck size={120} className="text-[var(--gf-color-warning)]" />
        </div>
        <div className="flex gap-4 relative z-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-[var(--ds-text-subtle)] leading-relaxed max-w-2xl font-medium">
              In the GateFlow system, an input focus state is a multi-layered event. We combine <strong>--ds-border-focused</strong> for the rim and <strong>--ds-glow-accent</strong> for the outer bloom. This creates a high-fidelity interaction that feels responsive and premium.
            </p>
          </div>
        </div>
      </section>
          ...
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-2 relative">
              ...
              <span className={cn(
                ...
                step >= i ? "text-[var(--ds-text-primary)]" : "text-[var(--ds-text-subtlest)]"
              )}>
            ...
            {i < 3 && (
              <div className={cn(
                ...
                step > i ? "bg-[var(--ds-background-brand-bold)]" : "bg-[var(--ds-border-subtle)]"
              )} />
            )}
          </React.Fragment>
        ))}
      ...
      <div className="flex-1 mt-6 relative">
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtle)]">First Name</Label>
                  <Input placeholder="Enter given name..." className="bg-[var(--ds-surface-subtle)] border-[var(--ds-border-subtle)] focus-visible:ring-[var(--ds-primary-accent)] h-11" />
                </div>
                <div className="flex flex-col gap-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtle)]">Last Name</Label>
                  <Input placeholder="Enter family name..." className="bg-[var(--ds-surface-subtle)] border-[var(--ds-border-subtle)] h-11" />
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtle)]">Institutional Email</Label>
                <div className="relative">
                  <Input defaultValue="user@gateflow.corp" className="bg-[var(--ds-surface-subtle)] border-[var(--gf-color-warning)] border-2 h-11 pr-24" />
          ...
          {step === 2 && (
            ...
            >
              <div className="flex flex-col gap-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--ds-text-subtle)]">Workspace Access Role</Label>
                <div className="grid grid-cols-2 gap-3">
                  {['Administrator', 'Scanner Lead'].map(role => (
                    <div key={role} className="p-4 rounded-xl border border-[var(--ds-border-subtle)] bg-[var(--ds-surface-subtle)] hover:bg-[var(--ds-background-selected)] hover:text-[var(--ds-text-selected)] cursor-pointer transition-all flex items-center justify-between group">
          ...
          {step === 3 && (
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-xl font-black uppercase tracking-tight text-[var(--ds-text-primary)]">Protocol Established</h3>
                <p className="text-xs text-[var(--ds-text-subtle)] font-medium">New tenant profile has been successfully generated.</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
