...
export default async function HomePage() {
  ...
  if (!claims) {
    ...
    // For now, let's just show a dummy dashboard or a message
  }

  // Fallback userId for development if not logged in
  const userId = claims?.sub || 'dev-resident-id';
  
  // Fetch Resident's Unit
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="text-center">
          <h1 className="text-xl font-bold mb-2">No Unit Assigned</h1>
          <p className="text-slate-500">Please contact management to assign your unit.</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-6 pb-24">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="font-semibold text-slate-900">{unit?.name || 'Unit A-101'}</p>
          ...
          <div className="flex items-center justify-center mb-4">
            <div className="relative h-24 w-24">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-bold text-slate-900">{quota.used}/{quota.quota}</span>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {v.accessRule?.type === 'ONETIME' ? 'One-time access' : 
                         v.accessRule?.type === 'RECURRING' ? 'Recurring access' : 
                         'Permanent access'}
                      </p>
                    </div>
                  </div>
