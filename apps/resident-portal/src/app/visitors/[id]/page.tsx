...
export default async function VisitorDetailPage({ params }: { params: { id: string } }) {
  ...
  const isActive = visitor.qrCode.isActive && (!visitor.qrCode.expiresAt || visitor.qrCode.expiresAt > new Date());
  ...
  const timeStr = (visitor.accessRule?.startTime && visitor.accessRule?.endTime)
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium">{visitor.qrCode.currentUses} / {visitor.qrCode.maxUses || '∞'}</p>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium">{format(new Date(visitor.createdAt), 'MMM dd, yyyy')}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-slate-500 uppercase font-semibold">ID</p>
                <p className="text-sm font-mono text-slate-400">#{visitor.id.slice(-6).toUpperCase()}</p>
              </div>
            </div>
          </div>
