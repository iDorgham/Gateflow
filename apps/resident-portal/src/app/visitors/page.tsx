    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-4 pb-24">
        ...
        <div className="space-y-3">
          {visitors.length > 0 ? (
            visitors.map((v) => {
              const isActive = v.qrCode.isActive && (!v.qrCode.expiresAt || v.qrCode.expiresAt > new Date());
              return (
                <div key={v.id} className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`h-12 w-12 rounded-full flex items-center justify-center ${
                      v.isOpenQR ? 'bg-purple-100' : 'bg-blue-100'
                    }`}>
                      {v.isOpenQR ? (
                        <QrCode className={`h-6 w-6 ${v.isOpenQR ? 'text-purple-600' : 'text-blue-600'}`} />
                      ) : (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {v.accessRule?.type === 'ONETIME' ? 'One-time' : 
                         v.accessRule?.type === 'RECURRING' ? 'Recurring' : 
                         'Permanent'} • {v.qrCode.currentUses} uses
                      </p>
                    </div>
                  </div>
