...
import { RadioGroup, RadioGroupItem, Label, cn } from '@gateflow/ui';
...
export function AccessRuleSelector({
  value,
  onChange,
  className,
}: AccessRuleSelectorProps) {
  const rules = [
    {
      id: 'always',
      label: 'Always Allow',
          ...
          <div key={rule.id}>
            ...
            >
              <Icon
                className={cn(
                  'mb-3 h-6 w-6',
                  isActive ? rule.color : 'text-muted-foreground'
                )}
              />
              <div className="space-y-1 text-center">
                <p
                  className={cn(
                    ...
                    isActive && rule.id === 'blocked' ? 'text-danger' : ''
                  )}
                >
                  {rule.label}
                </p>
