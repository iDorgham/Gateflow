...
export const automationTools = {
  scheduleReport: tool({
    description:
      'Schedule an automated operational report (e.g., "Email me a visitor report every Friday").',
    inputSchema: z.object({
      name: z
        .string()
        .min(1)
        .describe(
          'Descriptive name for the automation (e.g. Weekly Visitor Summary)'
        ),
      reportType: z
        .enum(['VISITORS', 'SCANS', 'ALERTS', 'ANALYTICS'])
        .describe('The type of data to report on'),
      frequency: z
        .string()
        .describe(
          'Natural language frequency (e.g. "every Friday", "daily at 8am")'
        ),
    ...
    execute: async ({ name, reportType, frequency, format, recipients }) => {
      ...
      if (role !== 'ADMIN' && role !== 'TENANT_ADMIN' && role !== 'MANAGER') {
        return {
          error: 'Forbidden: You do not have permission to create automations.',
        };
      }
  ...
  exportDataNow: tool({
    ...
    inputSchema: z.object({
      dataType: z
        .enum(['VISITORS', 'SCANS', 'ALERTS'])
        .describe('The data set to export'),
      format: z.enum(['CSV', 'PDF']).default('CSV'),
      timeRange: z
        .string()
        .optional()
        .describe(
          'Natural language time range (e.g. "last 24 hours", "this month")'
        ),
    }),
