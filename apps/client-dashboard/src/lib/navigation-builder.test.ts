...
describe('navigation-builder', () => {
  ...
  it('should filter out items based on permissions', () => {
    const features = ORGANIZATION_FEATURES[OrganizationType.REAL_ESTATE];
    const restrictedPermissions = { ...mockPermissions, 'maintenance:view': false };
    const nav = buildSidebarNav(features, restrictedPermissions, t as any, false);
  ...
  it('should apply terminology overrides for Units in SCHOOL', () => {
    ...
    const schoolT = vi.fn((key, fallback) => {
      if (key === 'orgType.school.unitLabelPlural') return 'Classrooms';
      return fallback;
    });
    
    const nav = buildSidebarNav(features, mockPermissions, schoolT as any, false);
