...
export function ProjectWizard({
  ...
  const [gates, setGates] = useState([
    { id: 'default-1', name: 'Main Gate', location: '' },
  ]);
  ...
  const removeGate = (id: string) => {
    if (gates.length <= 1)
      return toast.error('A project must have at least one gate.');
      ...
      <DialogContent className="max-w-5xl sm:rounded-2xl p-0 overflow-hidden bg-background border-border">
        ...
        <div className="bg-gradient-to-br from-primary/5 via-transparent to-background p-6 border-b border-border/50">
          <DialogHeader>
            ...
            <DialogDescription className="text-base font-medium text-muted-foreground">
              Define the structure, resources, and access parameters of your new
              project.
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.title} className="flex items-center">
                <div
                  className={`flex items-center gap-3 ${isActive ? 'opacity-100' : 'opacity-40 grayscale'} transition-all`}
                >
                  <div
                    className={`h-10 w-10 rounded-full flex items-center justify-center font-bold ${isActive ? 'bg-primary text-primary-foreground shadow-md' : isCompleted ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}
                  >
                    {isCompleted ? (
                ...
                {i < steps.length - 1 && (
                  <div
                    className={`w-8 sm:w-16 h-px mx-4 ${isCompleted ? 'bg-primary' : 'bg-border'}`}
                  />
                )}
              </div>
            );
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className="min-h-[80px] resize-none rounded-xl bg-muted/50"
                />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setDetails({ ...details, logoUrl: e.target.value })
                    }
                    placeholder="https://..."
                    className="h-11 rounded-xl bg-muted/50"
                  />
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                    <ImageIcon className="h-3 w-3" /> Cover URL
                  </Label>
                  <Input
                    type="url"
                    value={details.coverUrl}
                    onChange={(e) =>
                      setDetails({ ...details, coverUrl: e.target.value })
                    }
                    placeholder="https://..."
                    className="h-11 rounded-xl bg-muted/50"
                  />
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Define physical checkpoints for this project.
                  </p>
                </div>
                <Button
                  onClick={addGate}
                  variant="secondary"
                  size="sm"
                  className="gap-2 rounded-xl font-bold bg-primary/10 text-primary hover:bg-primary/20"
                >
              ...
              <div className="space-y-3">
                {gates.map((g) => (
                  <div
                    key={g.id}
                    className="flex flex-col sm:flex-row items-center gap-3 p-3 rounded-2xl border border-border/50 bg-muted/20"
                  >
                    <div className="w-full space-y-1">
                      ...
                      <Input
                        value={g.name}
                        onChange={(e) =>
                          updateGate(g.id, 'name', e.target.value)
                        }
                    ...
                    <div className="w-full space-y-1">
                      ...
                      <Input
                        value={g.location}
                        onChange={(e) =>
                          updateGate(g.id, 'location', e.target.value)
                        }
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Upload a CSV to bulk-create properties or offices.
                  </p>
                </div>
              </div>
                ...
                {units.length > 0 ? (
                  <div className="absolute inset-0 overflow-y-auto p-4 bg-background/95 backdrop-blur-sm z-20">
                    <div className="flex items-center justify-between mb-4">
                      <p className="font-bold text-primary">
                        {units.length} Units Parsed Ready
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                    ...
                    <div className="grid grid-cols-3 gap-2 text-left">
                      {units.slice(0, 50).map((u, i) => (
                        <div
                          key={i}
                          className="p-2 border border-border/50 rounded-lg bg-muted/20 text-xs"
                        >
                          <span className="font-bold">{u.name}</span>{' '}
                          <span className="text-muted-foreground block text-[10px]">
                            {u.type} • {u.qrQuota} QR/mo
                          </span>
                        </div>
                      ))}
                      {units.length > 50 && (
                        <div className="p-2 flex items-center justify-center text-xs font-bold text-muted-foreground">
                          +{units.length - 50} more...
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <Button
                      type="button"
                      variant="secondary"
                      className="mt-6 rounded-xl font-bold z-0 pointer-events-none"
                    >
                      Browse Files
                    </Button>
                  </>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Upload a CSV to bulk-create residents or staff.
                  </p>
                </div>
              </div>
              ...
              <div className="relative flex-1 rounded-2xl border-2 border-dashed border-border/60 bg-muted/10 hover:bg-muted/30 transition-colors flex flex-col items-center justify-center p-6 text-center group overflow-hidden">
                ...
                {contacts.length > 0 ? (
                  <div className="absolute inset-0 overflow-y-auto p-4 bg-background/95 backdrop-blur-sm z-20">
                    <div className="flex items-center justify-between mb-4">
                      <p className="font-bold text-primary">
                        {contacts.length} Contacts Parsed Ready
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                    ...
                    <div className="space-y-2 text-left">
                      {contacts.slice(0, 10).map((c, i) => (
                        <div
                          key={i}
                          className="px-3 py-2 border border-border/50 rounded-lg bg-muted/20 text-sm flex justify-between items-center"
                        >
                      ...
                      {contacts.length > 10 && (
                        <div className="text-center text-xs font-bold text-muted-foreground mt-4">
        ...
        <div className="p-6 border-t border-border/50 bg-muted/10 flex items-center justify-between rounded-b-2xl">
          <Button
            ...
            className="rounded-xl font-bold bg-background gap-2 min-w-[100px]"
          >
          ...
          <div className="flex gap-3">
            ...
            {step < steps.length - 1 ? (
              <Button
                onClick={handleNext}
                disabled={step === 0 && !details.name.trim()}
                className="gap-2 text-primary-foreground rounded-xl bg-primary font-bold shadow-sm hover:bg-primary/90 min-w-[120px]"
              >
              ...
              <Button
                ...
                disabled={isPending}
                className="gap-2 text-primary-foreground rounded-xl bg-primary font-bold shadow-sm hover:bg-primary/90 min-w-[160px]"
              >
                {isPending ? (
                  <>
