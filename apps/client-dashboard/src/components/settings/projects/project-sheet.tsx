...
export function ProjectSheet({ mode, project, children }: ProjectSheetProps) {
  ...
  const [gateMode, setGateMode] = useState<GateMode>(
    (project?.gateMode as GateMode) || 'MULTI'
  );
    ...
    if (open) {
      ...
      const fetchResources = async () => {
        ...
        if (mode === 'edit' && project) {
          setSelectedGates(
            res.gates.filter((g) => g.projectId === project.id).map((g) => g.id)
          );
          setSelectedUnits(
            res.units.filter((u) => u.projectId === project.id).map((u) => u.id)
          );
        } else {
          setSelectedGates([]);
          setSelectedUnits([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(
          result.error || t('projects.saveError', 'Failed to save project')
        );
      }
    });
  };
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="sm:max-w-md">
        <SheetHeader>
          <SheetTitle>
            {mode === 'create'
              ? t('projects.addTitle', 'Add New Project')
              : t('projects.editTitle', 'Edit Project')}
          </SheetTitle>
          <SheetDescription>
            {t(
              'projects.sheetDescription',
              'Define project scope and map physical resources.'
            )}
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 py-6">
          <div className="space-y-2">
            <Label htmlFor="name">
              {t('projects.fields.name', 'Project Name')}
            </Label>
            <Input
              id="name"
              value={name}
              ...
              placeholder={t(
                'projects.fields.namePlaceholder',
                'e.g. West Wing'
              )}
              className="font-bold"
            />
          ...
          <div className="space-y-2">
            <Label htmlFor="description">
              {t('projects.fields.description', 'Description')}
            </Label>
            <Textarea
              id="description"
              value={description}
              ...
              placeholder={t(
                'projects.fields.descriptionPlaceholder',
                'Briefly describe this project scope...'
              )}
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              value={gateMode}
              onValueChange={(v) => setGateMode(v as GateMode)}
              className="flex gap-4"
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="SINGLE" id="mode-single" />
                <Label htmlFor="mode-single" className="font-medium">
                  {t('projects.modes.single', 'Single Gate')}
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="MULTI" id="mode-multi" />
                <Label htmlFor="mode-multi" className="font-medium">
                  {t('projects.modes.multi', 'Multi-Gate')}
                </Label>
              </div>
            </RadioGroup>
          </div>
          ...
          <div className="space-y-2">
            ...
            <MultiSelect
              ...
              placeholder={t(
                'projects.fields.selectGates',
                'Select gates for this project...'
              )}
            />
          </div>
