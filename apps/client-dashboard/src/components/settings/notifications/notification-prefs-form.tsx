...
export function NotificationPrefsForm({
  ...
  const updateChannel = (
    key: keyof NotificationConfig['channels'],
    val: boolean
  ) => {
    setConfig((prev) => ({
      ...prev,
  ...
  const events: {
  ...
  }[] = [
    {
      ...
      desc: 'Notify when a visitor is successfully admitted',
    },
    {
      key: 'scanFailed',
      label: 'Scan Denied',
      icon: <ShieldAlert className="h-4 w-4 text-destructive" />,
      desc: 'Alert when a scan attempt is rejected or invalid',
      severity: 'high',
    },
    {
      key: 'qrExpired',
      ...
      desc: 'Notify when active QR codes reach their expiry',
      severity: 'medium',
    },
    {
      key: 'newMember',
      ...
      desc: 'Notify admins when a team invitation is accepted',
    },
    {
      key: 'systemAlerts',
      label: 'System Alerts',
    ...
    <div className="space-y-8">
      ...
      <Card className="rounded-2xl border-border shadow-none">
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Select which events generate notifications.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 pt-4">
          {events.map((ev) => (
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {ev.severity === 'high' && (
                      <Badge
                        variant="outline"
                        className="text-[9px] font-black uppercase bg-destructive/5 text-destructive border-destructive/20 py-0 px-1.5"
                      >
                        Critical
                      </Badge>
                    )}
                    {ev.severity === 'medium' && (
                      <Badge
                        variant="outline"
                        className="text-[9px] font-black uppercase bg-warning/5 text-warning border-warning/20 py-0 px-1.5"
                      >
                        Important
                      </Badge>
                    )}
