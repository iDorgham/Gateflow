...
const ACTION_MAP: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  INVITE_MEMBER: { label: 'Invited Member', icon: UserPlus, color: 'text-primary bg-primary/10' },
  REVOKE_INVITATION: { label: 'Revoked Invite', icon: Trash2, color: 'text-orange-500 bg-orange-500/10' },
  UPDATE_MEMBER_ROLE: { label: 'Updated Role', icon: Shield, color: 'text-blue-500 bg-blue-500/10' },
  REMOVE_MEMBER: { label: 'Removed Member', icon: Trash2, color: 'text-red-500 bg-red-500/10' },
  REVOKE_SESSIONS: { label: 'Force Logout', icon: RefreshCw, color: 'text-purple-500 bg-purple-500/10' },
  CREATE_ROLE: { label: 'Created Role', icon: Key, color: 'text-emerald-500 bg-emerald-500/10' },
  UPDATE_ROLE: { label: 'Updated Role Def', icon: Key, color: 'text-blue-500 bg-blue-500/10' },
  DELETE_ROLE: { label: 'Deleted Role', icon: Trash2, color: 'text-red-500 bg-red-500/10' },
};
  ...
  if (logs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-border rounded-2xl bg-muted/5">
        ...
        <h3 className="text-sm font-bold uppercase tracking-tight">No activity recorded</h3>
      ...
      <Table>
        <TableHeader className="bg-muted/30">
          <TableRow className="hover:bg-transparent border-border/50">
            ...
            <TableHead className="text-[10px] font-black uppercase tracking-widest h-10">Target</TableHead>
            <TableHead className="text-[10px] font-black uppercase tracking-widest h-10 text-right">Time</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {logs.map((log) => {
            const action = ACTION_MAP[log.action] || { label: log.action, icon: FileText, color: 'text-gray-500 bg-gray-500/10' };
                  ...
                  <div className="flex flex-col">
                    ...
                    {log.metadata && (
                      <span className="text-[9px] text-muted-foreground/70 font-medium">
                        {JSON.stringify(log.metadata).length > 2 ? JSON.stringify(log.metadata) : ''}
                      </span>
                    )}
                  </div>
                </TableCell>
