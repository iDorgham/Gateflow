...
async function fetchIncidents(
  ...
  const res = await fetch(buildIncidentsUrl(filters, groupBy), {
    credentials: 'include',
  });
  ...
  return json.data.map((r: IncidentsRow) => ({
    label: getLabel(r),
    count: r.count,
  }));
}
...
export function IncidentsChart({
  ...
  const noDataMessage =
    groupBy === 'gate'
      ? t(
          'analytics.noGateActivityInRange',
          'No denied scans by gate in this period.'
        )
  ...
  return (
    <AnalyticsChartCard
      ...
      tooltip={t(
        'analytics.incidentsDesc',
        'Denied scans by gate or operator.'
      )}
      loading={isLoading}
      className={className}
