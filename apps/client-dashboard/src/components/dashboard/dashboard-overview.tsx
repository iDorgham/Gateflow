...
export async function DashboardOverview({
  ...
  const [totalQRs, scansToday, activeGates, teamSize, recentScans] =
    await Promise.all([
      prisma.qRCode.count({
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            {t('overview.title', { defaultValue: 'Dashboard' })}
          </h1>
          ...
          {recentScans.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/30 py-10 text-center">
              ...
              <p className="text-sm font-medium text-foreground">
                {t('overview.noScans', { defaultValue: 'No scans yet' })}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {t('overview.noScansDesc', {
                  defaultValue:
              ...
              <Button variant="outline" size="sm" asChild className="mt-4">
                <Link href="/dashboard/qrcodes/create">
                  {t('overview.createFirst', {
                    defaultValue: 'Create your first QR code',
                  })}
                </Link>
              </Button>
            </div>
              ...
              {recentScans.map((scan) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {new Date(scan.scannedAt).toLocaleTimeString(locale, {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </time>
