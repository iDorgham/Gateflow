...
export function ScansFilters({
  ...
  const activeFilters: { label: string; key: string }[] = [
    sp.get('q') ? { label: `QR: "${sp.get('q')}"`, key: 'q' } : null,
    sp.get('status')
      ? { label: `Status: ${STATUS_LABELS[sp.get('status')!] ?? sp.get('status')}`, key: 'status' }
      : null,
    urlProject && urlProject !== 'all'
      ? {
          label: `Project: ${projects.find((p) => p.id === urlProject)?.name ?? urlProject}`,
          key: 'project',
        }
      : null,
    sp.get('gate')
      ? { label: `Gate: ${gates.find((g) => g.id === sp.get('gate'))?.name ?? sp.get('gate')}`, key: 'gate' }
      : null,
    sp.get('userId')
      ? {
          label: `Operator: ${operators.find((u) => u.id === sp.get('userId'))?.name ?? 'User'}`,
          key: 'userId',
        }
      : null,
    sp.get('deviceId') ? { label: `Device: ${sp.get('deviceId')}`, key: 'deviceId' } : null,
    sp.get('dateFrom') ? { label: `From: ${sp.get('dateFrom')}`, key: 'dateFrom' } : null,
    sp.get('dateTo') ? { label: `To: ${sp.get('dateTo')}`, key: 'dateTo' } : null,
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <p className="text-sm text-slate-500 dark:text-slate-400 whitespace-nowrap">
            Showing{' '}
            <span className="font-medium text-slate-700 dark:text-slate-300">{filteredCount.toLocaleString()}</span>
            {' '}of{' '}
