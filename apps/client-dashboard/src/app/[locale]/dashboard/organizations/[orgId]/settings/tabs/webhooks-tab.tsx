...
export function WebhooksTab({
  ...
  const handleCreate = async () => {
    const url = prompt(
      t(
        'settings.webhooks.promptUrl',
        'Enter the Webhook URL (must start with https://):'
      )
    );
    if (!url) return;
    if (!url.startsWith('https://')) {
      return toast.error(
        t(
          'settings.webhooks.errors.httpsRequired',
          'Secure HTTPS required for endpoint registration.'
        )
      );
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          t(
            'settings.webhooks.success.registered',
            'Webhook endpoint handshake successful.'
          )
        );
      } else {
        toast.error(
          data.message ||
            t(
              'settings.webhooks.errors.registrationFailed',
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        t(
          'settings.webhooks.confirmRevoke',
          'DANGER: Revoke this webhook? Event streams will be terminated immediately. Data loss may occur for undelivered events.'
        )
      )
    )
      return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          t('settings.webhooks.success.revoked', 'Stream endpoint revoked.')
        );
      } else {
        toast.error(
          t('settings.webhooks.errors.revokeFailed', 'Revocation failed.')
        );
      }
    } catch {
      toast.error(t('common.errors.network', 'Network connectivity issue.'));
  ...
  const toggleWebhook = async (id: string, currentlyActive: boolean) => {
    try {
      ...
      if (res.ok) {
        setWebhooks(
          webhooks.map((w) =>
            w.id === id ? { ...w, isActive: !currentlyActive } : w
          )
        );
        toast.success(
          currentlyActive
            ? t('settings.webhooks.success.paused', 'Stream paused.')
            : t('settings.webhooks.success.activated', 'Stream resuming.')
        );
      }
    } catch {
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground uppercase">
              {t('settings.webhooks.title', 'Real-time Event Streams')}
            </h1>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                ...
                {t(
                  ...
                  'Stream gate telemetry and security events directly to your backend infrastructure.'
                )}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {webhooks.length === 0 ? (
          <Card className="rounded-2xl border-dashed-4 border-2 border-border bg-muted/20">
            <CardContent className="py-24 text-center">
              ...
              <h3 className="text-lg font-black text-foreground uppercase tracking-tight">
                {t(
                  'settings.webhooks.noActiveStreams',
                  'No Active Telemetry Streams'
                )}
              </h3>
              <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-2 leading-relaxed font-medium">
                {t(
                  'settings.webhooks.emptyDesc',
                  'Synchronize your physical access nodes with your digital ecosystem. Register an HTTPS endpoint to begin receiving push telemetry.'
                )}
              </p>
              <Button
                onClick={handleCreate}
                className="mt-8 rounded-xl font-black uppercase tracking-widest text-[10px] bg-primary h-12 px-10 shadow-lg shadow-primary/20 hover:-translate-y-0.5 transition-all"
              >
                {t(
                  'settings.webhooks.registerFirst',
          ...
          webhooks.map((wh) => (
            <Card
              key={wh.id}
              className="rounded-2xl border border-border shadow-sm overflow-hidden bg-card transition-all hover:border-primary/20 hover:shadow-xl group"
            >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                  <div className="flex items-center gap-6 min-w-0">
                    ...
                    <div className="min-w-0 space-y-1">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge
                          variant="outline"
                          className="h-5 rounded-full px-2 text-[8px] font-black uppercase border-success/20 text-success bg-success/5"
                        >
                          SSL SECURED
                        </Badge>
                      </div>
                      <div className="flex items-center gap-4">
                        <Badge
                          variant="outline"
                          className="text-[9px] font-black uppercase border-border bg-muted/50 py-0.5 px-3 tracking-widest"
                        >
                  ...
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3 pr-6 border-r border-border rtl:border-r-0 rtl:border-l rtl:pl-6">
                      ...
                      >
                        {wh.isActive
                          ? t('settings.webhooks.streaming', 'Streaming')
                          : t('settings.webhooks.pausedStatus', 'Paused')}
                      </span>
                      <Switch
                        checked={wh.isActive}
                        onCheckedChange={() =>
                          toggleWebhook(wh.id, wh.isActive)
                        }
                      />
                    </div>
                    <Button
              ...
              <CardContent className="p-0 bg-muted/20">
                <div className="px-8 py-6">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[8px] font-black uppercase tracking-widest border-border text-muted-foreground/40"
                    >
                      Queue: Optimal
                    </Badge>
                  </div>
                  <div className="space-y-3">
                    {wh.deliveries.length === 0 ? (
                      ...
                      wh.deliveries.map((delivery) => (
                        <div
                          key={delivery.id}
                          className="flex items-center justify-between p-4 rounded-xl bg-background border border-border/50 shadow-sm transition-all hover:bg-muted/5 group/row"
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={cn(
                          ...
                          <div className="flex items-center gap-6">
                            ...
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 rounded-lg text-muted-foreground/20 group-hover/row:text-primary transition-colors"
                            >
