...
export function SettingsClient(props: SettingsClientProps) {
  ...
  const TABS_ALL = [
    ...
    },
    {
      ...
      icon: LayoutGrid,
      component: <div className="p-12 text-center text-muted-foreground">Resident settings coming soon to v6...</div>, // Fallback for now
    },
    {
      id: 'team',
  ...
  // Filter tabs by vertical config
  const TABS = TABS_ALL.filter(tab => features.settings.visibleTabs.includes(tab.id));

  const validIds = TABS.map(t => t.id);
  const initialTab = validIds.includes(tabParam) ? tabParam : (validIds[0] || 'workspace');
  ...
  // Removed static TABS definition
      ...
      <div className="border-b border-[var(--ds-border)] bg-[var(--ds-surface-raised)] px-6 py-5 mb-0">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-xl font-bold tracking-tight text-[var(--ds-text)] uppercase">
            {t(features.terminology.orgLabel)} {t('settings.gateflowTitleSuffix', 'Setup')}
          </h1>
        </div>
