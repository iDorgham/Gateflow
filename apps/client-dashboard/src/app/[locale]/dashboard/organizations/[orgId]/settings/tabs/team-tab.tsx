    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground uppercase">
              {t('settings.team.title', 'Human Resources & Access Controls')}
            </h1>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                ...
                {t(
                  ...
                  'Manage the administrative team and define granular permission matrices.'
                )}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            onClick={() =>
              toast.info(
                t(
                  'settings.team.messages.restricted',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map((member) => {
          ...
          const roleData =
            ROLE_PERMISSIONS[member.role] || ROLE_PERMISSIONS.TENANT_USER;
          const RoleIcon = roleData.icon;

          return (
            <Card
              key={member.id}
              className="relative group flex flex-col rounded-2xl border border-border bg-card transition-all duration-300 hover:shadow-2xl hover:border-primary/20 hover:-translate-y-1 overflow-hidden"
            >
              <CardHeader className="pb-4 pt-6 px-6 flex flex-row items-start justify-between">
                ...
                <Badge
                  className={cn(
                    'rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[0.15em] shadow-sm border',
                    roleData.color
                  )}
                >
                ...
                <div className="space-y-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {member.id === currentUserId && (
                      <Badge
                        variant="outline"
                        className="text-[8px] border-primary/30 text-primary uppercase font-black bg-primary/5"
                      >
                        YOU
                      </Badge>
                    )}
                ...
                <div className="mt-8 pt-5 border-t border-border/50 flex items-center justify-between">
                  ...
                  {member.id !== currentUserId && (
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-10 w-10 rounded-xl text-muted-foreground/30 hover:text-destructive hover:bg-destructive/10 transition-all active:scale-90"
                    >
        ...
        {/* Empty Slot */}
        <button className="group flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-border bg-muted/20 p-8 transition-all hover:border-primary/40 hover:bg-primary/5 active:scale-95">
          ...
          <div className="text-center">
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground/50 group-hover:text-primary transition-colors">
              Expand Operations
            </p>
            <p className="text-[9px] font-bold text-muted-foreground/30 uppercase tracking-tighter mt-1">
              Available seats: 4/5
            </p>
          </div>
        </button>
