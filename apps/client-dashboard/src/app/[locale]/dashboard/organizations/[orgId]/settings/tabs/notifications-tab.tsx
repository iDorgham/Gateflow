...
export function NotificationsTab() {
  ...
  function handleSave() {
    startTransition(async () => {
      ...
      toast.success(
        t(
          'settings.notifications.success.saved',
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground uppercase">
              {t('settings.notifications.title', 'Alerts & Notifications')}
            </h1>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                ...
                {t(
                  ...
                  'Manage external communications and system alerts.'
                )}
              </span>
            </div>
          </div>
      ...
      <Card className="rounded-2xl border border-border shadow-sm overflow-hidden bg-card">
        <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
          <CardTitle className="text-base font-bold uppercase tracking-tight flex items-center gap-2">
            ...
            {t(
              'settings.general.sections.notifications',
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-border/50">
            {[
              {
                id: 'email-alerts',
                label: t('settings.general.notif.email', 'Email Alerts'),
                desc: t(
                  'settings.general.notif.emailDesc',
                  'Receive daily summaries and critical scan alerts via email.'
                ),
                icon: Mail,
                checked: true,
              },
              {
                id: 'push-notif',
                label: t('settings.general.notif.push', 'Push Notifications'),
                desc: t(
                  'settings.general.notif.pushDesc',
                  'Get real-time browser alerts for immediate gate activity.'
                ),
                icon: Smartphone,
                checked: false,
              },
              {
                id: 'audit-notif',
                label: t('settings.general.notif.audit', 'Audit Logs'),
                desc: t(
                  'settings.general.notif.auditDesc',
                  'Notify me when any administrative changes are detected.'
                ),
                icon: Monitor,
                checked: true,
              },
              ...
              return (
                <div
                  key={item.id}
                  className="flex items-center justify-between p-8 hover:bg-muted/5 transition-colors group"
                >
                  <div className="flex items-center gap-6">
