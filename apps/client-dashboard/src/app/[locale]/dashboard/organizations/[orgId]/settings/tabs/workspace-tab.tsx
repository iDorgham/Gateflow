...
export function WorkspaceTab({ org }: { org: OrgData }) {
  ...
  const [requiredIdentityLevel, setRequiredIdentityLevel] = useState(
    org.requiredIdentityLevel ?? 0
  );
  ...
  const [visitorHistoryRetentionMonths, setVisitorHistoryRetentionMonths] =
    useState<string>(
      org.visitorHistoryRetentionMonths != null
        ? String(org.visitorHistoryRetentionMonths)
        : ''
    );
  const [idArtifactRetentionMonths, setIdArtifactRetentionMonths] =
    useState<string>(
      org.idArtifactRetentionMonths != null
        ? String(org.idArtifactRetentionMonths)
        : ''
    );
  const [incidentRetentionMonths, setIncidentRetentionMonths] =
    useState<string>(
      org.incidentRetentionMonths != null
        ? String(org.incidentRetentionMonths)
        : ''
    );
  const [maskResidentNameOnLandingPage, setMaskResidentNameOnLandingPage] =
    useState(org.maskResidentNameOnLandingPage ?? false);
  const [showUnitOnLandingPage, setShowUnitOnLandingPage] = useState(
    org.showUnitOnLandingPage ?? true
  );
  ...
  function handleSave() {
    if (!name.trim())
      return toast.error(
        t(
          'settings.workspace.errors.nameRequired',
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        toast.success(
          t(
            'settings.workspace.success.settingsSaved',
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setTimeout(() => {
        toast.success(
          t(
            'settings.workspace.success.logoSimulated',
            'Logo successfully stored in ecosystem.'
          )
        );
  ...
  function handleSaveRetention() {
    ...
    updateRetentionAndPrivacyAction({
      ...
      scanLogRetentionMonths: scanLogRetentionMonths
        ? parseInt(scanLogRetentionMonths, 10)
        : null,
      visitorHistoryRetentionMonths: visitorHistoryRetentionMonths
        ? parseInt(visitorHistoryRetentionMonths, 10)
        : null,
      idArtifactRetentionMonths: idArtifactRetentionMonths
        ? parseInt(idArtifactRetentionMonths, 10)
        : null,
      incidentRetentionMonths: incidentRetentionMonths
        ? parseInt(incidentRetentionMonths, 10)
        : null,
      ...
      if (res.success)
        toast.success(
          t('settings.workspace.success.settingsSaved', 'Settings saved.')
        );
      else
        toast.error(
          res.message || t('common.errors.generic', 'Operation failed.')
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 pb-8 border-b border-border/50">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            size="sm"
            className="rounded-xl h-10 px-5 font-bold uppercase tracking-widest text-[10px] gap-2 border-border border-2"
          >
          ...
          <Card className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
              ...
              <CardDescription className="text-xs">
                {t(
                  'settings.workspace.systemParamsDesc',
                  'Define the fundamental operational parameters of your workspace.'
                )}
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t('settings.workspace.legalName', 'Workspace Descriptor')}
                  </Label>
                  <Input
                    value={name}
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t(
                      'settings.workspace.billingEmail',
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {org.plan !== 'ENTERPRISE' && (
                    <Badge
                      variant="outline"
                      className="text-[8px] font-black tracking-widest border-warning/20 text-warning uppercase bg-warning/5"
                    >
                      Enterprise Only
                    </Badge>
                  )}
                ...
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  {t(
                    'settings.workspace.domainNotice',
                    'Connect a custom SSL-secured domain to fully white-label your QR codes, guest portals, and automated communications.'
                  )}
                </p>
              </div>
            </CardContent>
          ...
          <Card className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
              <CardTitle className="text-base font-bold uppercase tracking-tight flex items-center gap-2">
                ...
                {t(
                  'settings.workspace.privacyRetention',
                  'Privacy & Data Retention'
                )}
              </CardTitle>
              <CardDescription className="text-xs">
                {t(
                  'settings.workspace.privacyRetentionDesc',
                  'Visitor identity levels, retention policies, and resident-facing options. After X months, data is eligible for deletion per policy.'
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="p-8 space-y-8">
              <div className="space-y-4">
                <Label
                  htmlFor="identity-level"
                  className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60"
                >
                  {t(
                    'settings.workspace.identityLevel',
                    'Default visitor identity level'
                  )}
                </Label>
                <NativeSelect
                  id="identity-level"
                  ...
                  onChange={(e) =>
                    setRequiredIdentityLevel(parseInt(e.target.value, 10))
                  }
                ...
                <p className="text-[10px] text-muted-foreground">
                  {t(
                    'settings.workspace.identityLevelHint',
                    'Gates can override. Level 1 requires ID photo capture at scan.'
                  )}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t(
                      'settings.workspace.scanLogRetention',
                      'Scan log retention (months)'
                    )}
                  </Label>
                  <NativeSelect
                    ...
                    onChange={(e) =>
                      setScanLogRetentionMonths(
                        e.target.value === 'indefinite' ? '' : e.target.value
                      )
                    }
                    className="h-11 rounded-xl"
                  >
                ...
                <div className="space-y-2">
                  ...
                  >
                    {t(
                      'settings.workspace.incidentRetention',
                      'Incident retention (months)'
                    )}
                  </Label>
                  <NativeSelect
                    id="incident-retention"
                    value={incidentRetentionMonths || 'indefinite'}
                    onChange={(e) =>
                      setIncidentRetentionMonths(
                        e.target.value === 'indefinite' ? '' : e.target.value
                      )
                    }
                    className="h-11"
                ...
                <div className="space-y-2">
                  <Label
                    htmlFor="artifact-retention"
                    className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60"
                  >
                    {t(
                      'settings.workspace.idArtifactRetention',
                      'ID artifact retention (months)'
                    )}
                  </Label>
                  <NativeSelect
                    id="artifact-retention"
                    value={idArtifactRetentionMonths || 'indefinite'}
                    onChange={(e) =>
                      setIdArtifactRetentionMonths(
                        e.target.value === 'indefinite' ? '' : e.target.value
                      )
                    }
                    className="h-11"
                ...
                <div className="space-y-2">
                  <Label
                    htmlFor="visitor-retention"
                    className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60"
                  >
                    {t(
                      'settings.workspace.visitorHistoryRetention',
                      'Visitor history retention (months)'
                    )}
                  </Label>
                  <NativeSelect
                    id="visitor-retention"
                    value={visitorHistoryRetentionMonths || 'indefinite'}
                    onChange={(e) =>
                      setVisitorHistoryRetentionMonths(
                        e.target.value === 'indefinite' ? '' : e.target.value
                      )
                    }
                    className="h-11"
              ...
              <div className="border-t border-border pt-6 space-y-4">
                <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 flex items-center gap-2">
                  ...
                  {t(
                    'settings.workspace.residentOptions',
                    'Resident-facing options'
                  )}
                </Label>
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onChange={(e) =>
                        setMaskResidentNameOnLandingPage(e.target.checked)
                      }
                    />
                    <Label
                      htmlFor="maskResidentName"
                      className="cursor-pointer text-sm"
                    >
                      {t(
                        'settings.workspace.maskResidentName',
                        'Mask resident name on guest landing page'
                      )}
                    </Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onChange={(e) =>
                        setShowUnitOnLandingPage(e.target.checked)
                      }
                    />
                    <Label
                      htmlFor="showUnitOnLanding"
                      className="cursor-pointer text-sm"
                    >
                      {t(
                        'settings.workspace.showUnitOnLanding',
                        'Show unit number on guest landing page'
                      )}
                    </Label>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 rounded-xl border border-destructive/10 bg-background shadow-sm ring-1 ring-destructive/[0.03]">
                <div className="space-y-1 text-center sm:text-left">
                  <p className="text-sm font-black text-destructive uppercase tracking-tight">
                    {t('settings.workspace.deprovision', 'Deprovision Node')}
                  </p>
                  <p className="text-[11px] text-muted-foreground leading-relaxed max-w-sm">
                    {t(
                      'settings.workspace.deprovisionDesc',
                      'Permanently purge all telemetry, projects, and access keys. This action cannot be reversed.'
                    )}
                  </p>
                </div>
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        t(
                          'common.confirmTerminal',
                          'DANGER: Confirm permanent node deprovisioning?'
                        )
                      )
                    ) {
                      toast.error(
                        t(
                          ...
                          'Action restricted to primary node owner.'
                        )
                      );
                    }
                  }}
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="p-6 rounded-2xl border border-primary/20 bg-primary/5 space-y-4">
            ...
            <p className="text-[11px] leading-relaxed text-primary/80 font-medium">
              {t(
                'settings.workspace.upgradeInsight',
                'Enterprise nodes unlock high-frequency scan APIs, bespoke SLAs, and advanced data residency controls.'
              )}
            </p>
            <Button
              variant="ghost"
              className="w-full text-primary hover:bg-primary/10 text-[10px] font-black uppercase tracking-widest"
            >
