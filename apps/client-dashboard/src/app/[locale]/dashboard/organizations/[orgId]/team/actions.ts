...
export async function removeMember(memberId: string): Promise<SimpleResult> {
  try {
    ...
    logger.info(
      `removeMember: Success - Removed user ${memberId} from org ${claims.orgId}`
    );
    return { success: true };
  } catch (error) {
...
export async function changeRole(
  ...
  try {
    ...
    await prisma.user.update({
      where: { id: memberId },
      data: { roleId: newRoleRecord.id },
    });
    logger.info(
      `changeRole: Success - Changed user ${memberId} role to ${newRole} in org ${claims.orgId}`
    );
    return { success: true };
  } catch (error) {
