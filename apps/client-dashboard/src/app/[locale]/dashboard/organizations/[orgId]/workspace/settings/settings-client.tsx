      ...
      <SidebarSection title="Custom Domains" icon={Globe}>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm ring-1 ring-slate-100/50 dark:ring-slate-700/50">
          <p className="text-[11px] leading-relaxed text-slate-500 dark:text-slate-400 font-medium lowercase first-letter:uppercase">
            White-label your workspace by connecting a custom domain. This will
            be used for all QR short links and your public gate pages.
          </p>
          <Button
            variant="outline"
            size="sm"
            className="mt-4 w-full gap-2 rounded-xl text-[11px] font-bold border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700"
          >
      ...
      header={
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
            General settings and organizational metadata for your workspace.
          </p>
        </div>
      }
      sidebar={sidebar}
        ...
        >
          ...
          <CardContent className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-3">
              <span
                className={cn(
                  'flex items-center gap-1.5 rounded-full px-4 py-1.5 text-sm font-bold shadow-sm',
                  planStyle.badge
                )}
              >
                <span
            ...
            <Button
              ...
              size="sm"
              asChild
              className="w-full sm:w-auto shrink-0 gap-2 rounded-lg font-semibold border-slate-300 dark:border-slate-600 hover:bg-white dark:hover:bg-slate-700 shadow-sm"
            >
        ...
        <Card className="rounded-2xl shadow-sm border-slate-200 dark:border-slate-700">
          ...
          <CardContent className="space-y-5">
            ...
            <Button
              onClick={save}
              disabled={isPending}
              className="w-full sm:w-auto gap-2 rounded-lg bg-blue-600 font-semibold shadow-sm hover:bg-blue-700"
            >
              {isPending ? (
                <>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-xl border border-red-200 dark:border-red-900/50 bg-white dark:bg-slate-800 p-5 shadow-sm">
              <div className="space-y-1">
                ...
                <p className="text-sm text-red-600/80 leading-relaxed max-w-md">
                  All data, including gates, QR codes, and scan logs, will be
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    ...
                    toast.info(
                      'Contact support@gateflow.site to delete your organization.'
                    );
                  }
                }}
              >
