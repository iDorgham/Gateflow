...
export function ProfileTab({ user: initialUser }: { user: ProfileUser }) {
  ...
  function submit() {
    if (!name.trim())
      return toast.error(
        t('settings.profile.errors.nameRequired', 'Name is required.')
      );
    startTransition(async () => {
      ...
      if (result?.success) {
        toast.success(
          t(
            'settings.profile.success.profileUpdated',
            'Profile telemetry updated.'
          )
        );
      } else {
        toast.error(
          result?.error ||
            t('settings.profile.errors.updateFailed', 'Update failed.')
        );
      }
    });
  }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setTimeout(() => {
        toast.success(
          t(
            'settings.profile.success.photoSimulated',
            'Photo upload simulated! Interface updated.'
          )
        );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="relative group">
            <div className="h-24 w-24 rounded-2xl bg-muted border-2 border-dashed border-border flex items-center justify-center overflow-hidden transition-all group-hover:border-primary/50 group-hover:bg-primary/5">
              <Avatar className="h-full w-full rounded-none">
                <AvatarImage
                  src={initialUser.avatarUrl || undefined}
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3">
              <Badge className="bg-primary/10 text-primary border-primary/20 font-bold px-3 py-1 uppercase text-[10px] tracking-widest">
                ...
                {t(
                  `settings.profile.roleLabels.${initialUser.role}`,
                  initialUser.role
                )}
              </Badge>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          ...
          <Card className="rounded-2xl border border-border shadow-sm bg-card overflow-hidden">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
              ...
              <CardDescription className="text-xs">
                {t(
                  'settings.profile.contactAndLinksDesc',
                  'Phone, company, website and social links.'
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="p-8 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  placeholder={t(
                    'settings.profile.socialPlaceholder',
          ...
          <Card className="rounded-2xl border border-border shadow-sm overflow-hidden bg-card">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
              <CardTitle className="text-base font-bold uppercase tracking-tight flex items-center gap-2">
                ...
                {t(
                  'settings.general.sections.localization',
                  'Regional & Language'
                )}
              </CardTitle>
              <CardDescription className="text-xs">
                {t(
                  'settings.general.localizationDesc',
                  'Configure how dates, times, and languages are displayed.'
                )}
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t('settings.general.language', 'Preferred Language')}
                  </Label>
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t('settings.general.timezone', 'System Timezone')}
                  </Label>
                  <div className="relative group text-foreground">
          ...
          <Card className="rounded-2xl border border-border shadow-sm bg-card overflow-hidden">
            <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
              ...
              <CardDescription className="text-xs">
                {t(
                  'settings.profile.securityDesc',
                  'Manage your access credentials and protect your node.'
                )}
              </CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent className="px-6 pb-6 space-y-5">
              <div className="p-4 rounded-xl bg-muted/30 border border-border/50 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <code className="text-[10px] font-mono text-muted-foreground bg-background px-2 py-0.5 rounded border border-border/50">
                    {initialUser.id.slice(0, 12)}...
                  </code>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors cursor-pointer group">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] text-muted-foreground/60 uppercase font-black">
                      2 Connected Devices
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors cursor-pointer group">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] text-success uppercase font-black">
                      Secured via HMAC
                    </span>
                  </div>
                </div>
          ...
          <Button
            variant="outline"
            className="w-full rounded-xl border-border h-12 font-bold uppercase tracking-widest text-[10px] text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all"
          >
  ...
  return (
    <Card className="rounded-2xl border border-border shadow-sm bg-card overflow-hidden">
      <CardHeader className="pb-4 pt-6 px-8 border-b border-border/50 bg-muted/5">
        ...
        <CardDescription className="text-xs">
          {t(
            'settings.profile.generalInfoDesc',
            'Update your public persona and system-wide identifiers.'
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <Label
              ...
              className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60"
            >
              {t('settings.profile.legalName', 'Full Identity')}
            </Label>
            <Input
              id="displayName"
              value={name}
        ...
        <div className="space-y-3">
          <Label
            ...
            className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60"
          >
            {t('settings.profile.bio', 'Personnel Bio')}
          </Label>
          <Textarea
            id="bio"
            placeholder={t(
              'settings.profile.bioPlaceholder',
              'Brief descriptions for your team members...'
            )}
            value={bio}
...
function ChangePasswordCard() {
  ...
  function submit() {
    if (!current || !next || !confirm)
      return toast.error(t('common.errors.required', 'All fields required.'));
    if (next.length < 8)
      return toast.error(
        t('common.errors.passwordLength', 'Min 8 characters required.')
      );
    if (next !== confirm)
      return toast.error(
        t('common.errors.passwordMismatch', 'Passwords do not match.')
      );
    ...
    startTransition(async () => {
      ...
      if (result?.success) {
        toast.success(
          t(
            'settings.profile.success.passwordUpdated',
            'Authentication key rotation successful.'
          )
        );
        setCurrent('');
        setNext('');
        ...
        toast.error(
          result?.error || t('common.errors.generic', 'Operation failed.')
        );
      }
    });
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          {
            id: 'curr',
            label: t('settings.profile.currentPwd', 'Current Key'),
            val: current,
            set: setCurrent,
          },
          {
            id: 'new',
            label: t('settings.profile.newPwd', 'New Key'),
            val: next,
            set: setNext,
          },
          {
            id: 'conf',
            label: t('settings.profile.confirmPwd', 'Verify Key'),
            val: confirm,
            set: setConfirm,
          },
        ].map((item) => (
