...
export function IntegrationsTab() {
  ...
  const INTEGRATIONS = [
    ...
    },
    {
      ...
      name: 'Zapier',
      description:
        'Connect GateFlow with 5,000+ apps to automate your entire security workflow.',
      icon: Zap,
      category: 'Automation',
      status: 'Connect',
      accent: 'orange',
    },
    {
      ...
      name: 'Discord',
      description:
        'Receive real-time notifications and gate check-in logs directly in Discord.',
      icon: MessageSquare,
      category: 'Messaging',
      status: 'Connect',
      accent: 'indigo',
    },
    {
      ...
      name: 'Airtable',
      description:
        'Automatically sync guest logs and QR code data to your Airtable bases.',
      icon: Database,
      category: 'Database',
      status: 'Connect',
      accent: 'blue',
    },
    {
      ...
      name: 'Webhooks Pro',
      description:
        'Advanced payload signing and retry logic for enterprise-grade integrations.',
      icon: Globe,
      category: 'Developer',
      status: 'Active',
      accent: 'violet',
    },
    {
      ...
      name: 'AI Analytics',
      description:
        'Leverage machine learning to detect anomalous access patterns and potential leaks.',
      icon: Bot,
      category: 'Security',
      status: 'Coming Soon',
      accent: 'slate',
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground uppercase">
              {t('settings.integrations.title', 'Ecosystem & Extensions')}
            </h1>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                ...
                {t(
                  ...
                  'Extend the capabilities of GateFlow by connecting to external services.'
                )}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {INTEGRATIONS.map((app) => {
          ...
          return (
            <Card
              key={app.id}
              className="group relative flex flex-col rounded-2xl border border-border bg-card transition-all duration-300 hover:shadow-xl hover:border-primary/20 overflow-hidden"
            >
              <CardHeader className="pb-4 pt-6 px-6">
                <div className="flex items-start justify-between">
                  ...
                  <Badge
                    variant="outline"
                    className="rounded-full px-2 py-0 text-[10px] font-bold uppercase tracking-widest border-border text-muted-foreground/60 bg-muted/5"
                  >
                    {app.category}
                  </Badge>
                </div>
        ...
        <button className="group flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-border bg-muted/30 p-8 transition-all hover:border-primary/40 hover:bg-primary/5 active:scale-95">
          ...
          <div className="text-center">
            <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground/60 group-hover:text-primary transition-colors">
              {t('settings.integrations.requestBridge', 'Request Bridge')}
            </p>
            <p className="text-[10px] font-semibold text-muted-foreground/40 uppercase tracking-tighter mt-1">
              {t(
                'settings.integrations.bespokeDesc',
                'Need a bespoke connection?'
              )}
            </p>
          </div>
        </button>
