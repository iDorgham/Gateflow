  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          t('settings.projects.success.purged', 'Project deleted gracefully')
        );
            ...
            <Card className="rounded-2xl border-border shadow-sm overflow-hidden">
              <CardContent className="py-20 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground max-w-sm mx-auto">
                  Projects help you organize your gates and QR codes. Create one
                  to get started.
                </p>
                <Button
                  className="mt-8 rounded-xl font-bold gap-2 shadow-sm"
                  onClick={() => setShowCreate(true)}
                >
                  <Plus className="h-4 w-4" />{' '}
                  {t('settings.projects.newProject', 'New Project')}
