    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            {topGates.length === 0 ? (
              <p className="text-sm text-slate-400">{t('analytics.noGateActivity')}</p>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  <Tooltip
                    contentStyle={{ fontSize: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}
                  />
                  <Bar dataKey="scans" name={t('analytics.scans')} fill="#8b5cf6" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {roleBreakdown.length === 0 ? (
              <p className="text-sm text-slate-400">{t('analytics.noRoleData')}</p>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie
        ...
        <Card>
          ...
          <CardContent>
            {qrTypeBreakdown.length === 0 ? (
              <p className="text-sm text-slate-400">{t('analytics.noScanDataInRange')}</p>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie
        ...
        <Card>
          ...
          <CardContent>
            {gateSuccessRates.length === 0 ? (
              <p className="text-sm text-slate-400">{t('analytics.noGateActivityInRange')}</p>
            ) : (
              <ResponsiveContainer
                width="100%"
            ...
            <div className="min-w-[560px]">
              ...
              {DOW_LABELS.map((dayKey, dow) => (
                <div key={dow} className="flex items-center">
                  <div className="w-10 shrink-0 text-right pr-2 text-[10px] text-slate-500">{t(`analytics.daysOfWeek.${dayKey}`)}</div>
