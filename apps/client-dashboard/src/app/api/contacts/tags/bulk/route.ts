  ...
  try {
    ...
    if (action === 'add') {
      const existing = await prisma.contactTag.findMany({
        // ignore-security-guard — validContactIds/validTagIds pre-filtered by organizationId above
        where: {
      ...
      const existingSet = new Set(
        existing.map((e) => `${e.contactId}:${e.tagId}`)
      );
      const toCreate: { contactId: string; tagId: string }[] = [];
      for (const contactId of validContactIds) {
        for (const tagId of validTagIds) {
          if (!existingSet.has(`${contactId}:${tagId}`))
            toCreate.push({ contactId, tagId });
        }
      }
    ...
    return NextResponse.json({
      success: true,
      updated: validContactIds.length,
    });
  } catch (error) {
