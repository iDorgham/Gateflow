...
const GetContactsQuerySchema = z.object({
  ...
  tagIds: z.string().optional().transform((s) => (s ? s.split(',').filter(Boolean) : undefined)),
  sort: z.enum(['firstName', 'lastName', 'visitsInRange', 'passesInRange', 'lastVisitInRange']).optional(),
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { unitId, format, from, to, search, unitType, gateId, projectId, tagIds, sort, sortDir, page, pageSize } =
      parsed.data;
    ...
    const [contacts, total] = await Promise.all([
      prisma.contact.findMany({
        ...
        orderBy: orderBy as Parameters<typeof prisma.contact.findMany>[0]['orderBy'],
    ...
    const data = contacts.map((c) => {
      ...
      for (const uid of unitIds) {
        ...
        if (agg) {
          ...
          if (agg.lastVisitInRange && (!lastVisitInRange || agg.lastVisitInRange > lastVisitInRange)) {
