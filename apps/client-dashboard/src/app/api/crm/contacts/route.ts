...
const GetContactsQuerySchema = z.object({
  ...
  sort: z.enum(['firstName', 'lastName', 'createdAt']).optional().default('firstName'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const parsed = GetContactsQuerySchema.safeParse(Object.fromEntries(searchParams));
    ...
    return NextResponse.json({ success: true, data: contacts, total, page, pageSize });
  } catch (error) {
    ...
    const contact = await prisma.contact.create({
      data: {
        ...
        units: unitIds?.length ? {
          create: await prisma.unit.findMany({
