...
// Use process.env.SETUP_SECRET in production. Hardcoded fallback for dev only.
const SECRET = process.env.SETUP_SECRET || 'gateflow-setup-2026';

// Pre-computed argon2id hash of "password123" (generated locally)
const PASSWORD_HASH =
  '$argon2id$v=19$m=65536,t=3,p=4$Abr48HQ+axKv5PKJMQmHUw$xMDgdrY4kGO9XUGY/apj8UcT+n1FaButa24uu1P1g9Q';
...
export async function GET(request: NextRequest) {
  ...
  if (!tenantAdminRole)
    return NextResponse.json(
      { error: 'TENANT_ADMIN role missing after seed' },
      { status: 500 }
    );

  // 2. Find or create org
  let org = await prisma.organization.findFirst({
    where: { email: 'org@selenadev.com' },
  });
  if (!org) {
    org = await prisma.organization.create({
      data: { name: 'Selena Dev', email: 'org@selenadev.com' },
    });
