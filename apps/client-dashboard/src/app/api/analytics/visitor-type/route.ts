...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rows = (await prisma.scanLog.groupBy({
      ...
      where,
      _count: { id: true },
    })) as { qrCodeId: string; _count: { id: number } }[];
    ...
    const typeByQr = new Map<string, string>(
      qrCodes.map((q) => [q.id, q.type])
    );
    ...
    const data: VisitorTypeRow[] = Array.from(byType.entries()).map(
      ([type, count]) => ({
        type,
        count,
      })
