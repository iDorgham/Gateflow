 ...
 * Stacked time series: New vs Returning visitors by date.
 * Stub: first-scan vs repeat not in schema; splits total count (e.g. 60% new / 40% returning).
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const data: NewVsReturningPoint[] = [];
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      data.push({
        date: key,
        newCount,
        returningCount: count - newCount,
      });
    }

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('GET /api/analytics/new-vs-returning error:', error);
