...
`.antigravity/agents/` — Role personas (planning, security, backend-api, frontend, etc.) and scenarios (code-review, security-audit, refactor). Adopt when starting a phase or task. Orchestrator: when to use subagents/MCP/CLI.

## Contracts

`.antigravity/contracts/` — Invariants that all code must satisfy:
- Multi-tenant scope (`organizationId`)
- Soft deletes (`deletedAt: null`)
- QR security (HMAC-SHA256)
- Auth, validation, secrets

See `.antigravity/contracts/CONTRACTS.md` for full details.
...
- `.antigravity/contracts/` — Code invariants
- `docs/plan/guidelines/PHASED_DEVELOPMENT_WORKFLOW.md` — Phased flow
- `.antigravity/skills/planner/SKILL.md` — Planning skill
