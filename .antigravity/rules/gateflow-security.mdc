...
- `.antigravity/rules/00-gateflow-core.mdc` — core security and multi‑tenancy rules.
- `.antigravity/contracts/CONTRACTS.md` — invariants for multi‑tenancy, soft deletes, QR security, auth, validation, secrets, pnpm, and imports.
- `.antigravity/skills/gf-security/SKILL.md` — detailed security skill (auth stack, RBAC, API checklist, QR + mobile security).
...
These invariants are defined in `.antigravity/rules/00-gateflow-core.mdc` and `.antigravity/contracts/CONTRACTS.md` and are repeated here for emphasis.
...
This checklist mirrors `.antigravity/skills/gf-security/SKILL.md` and must be followed for **all** new or modified API routes.
...
- **Adopt the security persona**
  ...
  - In Cursor, adopt the security role agent from `.antigravity/agents/roles/security.md` (via orchestrator) when available.
- **Load the right skills and docs**
  - Always load `.antigravity/skills/gf-security/SKILL.md` before changing auth, RBAC, QR, scanner sync, or any tenant‑scoped API.
  - Cross‑check invariants in `.antigravity/contracts/CONTRACTS.md` and `.antigravity/rules/00-gateflow-core.mdc`.
...
- **Multi‑CLI usage**
  - Multi‑CLI (e.g. Claude + Gemini) may be used for SECURITY phases **only** when the phase is complex or high‑risk (as per `AI_SKILLS_SUBAGENTS_RULES.md` and `.antigravity/skills/gf-planner/SKILL.md`).
