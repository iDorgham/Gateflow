...
- **After a CLI task**: record the result in `docs/development/learning/CLI_USAGE_AND_RESULTS.md` so future runs can analyze usage and improve decisions (see gf-guide and gf-dev for the recording step).
...
- **`docs/development/learning/GUIDE_PREFERENCES.md`** — Optional section **"CLI limits / usage"** or **"Today’s limits"**: user can add one line such as "Claude CLI near daily limit" or "prefer free tier today" so the guide and agents suggest alternatives.
- **`docs/development/learning/CLI_LIMITS_TRACKING.md`** (optional) — If present, the user can note current usage or "near limit" per tool (e.g. "Claude: 45/50 ops today"). Agents load this file when suggesting a CLI and prefer free-tier or Cursor when a paid tool is near its stated limit.
- **`docs/development/learning/CLI_USAGE_AND_RESULTS.md`** — Log of past usage and outcomes. When analyzing (e.g. weekly), use it to see which tools are used heavily and suggest rotating to free tier or Cursor to stay under limits.
...
1. **Load** `docs/development/learning/GUIDE_PREFERENCES.md` and, if it exists, `docs/development/learning/CLI_LIMITS_TRACKING.md`.
...
5. **After** a task completed with a CLI, remind or ensure one entry is added to `docs/development/learning/CLI_USAGE_AND_RESULTS.md` (date, CLI, task, outcome, notes).
...
- `docs/development/learning/GUIDE_PREFERENCES.md` — User preferences and optional "CLI limits / usage" or "My tools".
- `docs/development/learning/CLI_USAGE_AND_RESULTS.md` — Where to record each CLI task result for future analysis and better decisions.
- `.antigravity/skills/gf-guide/SKILL.md` — Guide uses this skill when suggesting CLIs.
- `.antigravity/skills/gf-dev/SKILL.md` — `/dev` uses this skill when choosing or invoking a CLI for a phase.
