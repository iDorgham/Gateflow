...
**Contract:** Offline QR verification never grants access. A scan that cannot
complete authoritative server validation may be stored as `pending` only when
the encrypted queue write succeeds with a real `gateId`; otherwise it is
rejected. Only a server `accepted` response with a persisted `scanId` may reach
the operator Pass decision or display ACCESS GRANTED.
...
This contract is enforced together with `.antigravity/rules/00-gateflow-core.mdc`, `.antigravity/rules/gateflow-security.mdc`, and `.antigravity/skills/gf-security/SKILL.md`.

---

## 10. SECURITY specialist phases

**Contract:** When a plan phase or task has **Primary role: SECURITY** (see `docs/development/guidelines/SUBAGENT_HIERARCHY.md`):

- The implementing agent must:
  - Load `.antigravity/skills/gf-security/SKILL.md` and `.antigravity/contracts/CONTRACTS.md` before making changes.
...
- Ignore or override SECURITY contracts in this file, `.antigravity/rules/00-gateflow-core.mdc`, or `.antigravity/rules/gateflow-security.mdc`
