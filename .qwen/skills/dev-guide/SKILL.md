...
**Reference:** `.antigravity/skills/gf-mcp/SKILL.md`, `docs/MCP_SETUP.md`
...
2. **Run via `/dev`**:
   ...
   - When **Preferred tool** is a CLI (Claude CLI, Gemini CLI, OpenCode CLI, **Kiro CLI**, **Kilo CLI**, **Qwen CLI**), follow `.antigravity/skills/multi-cli-cursor-workflow/SKILL.md`: invoke that CLI with the phase prompt and use Cursor for file edits (or run the phase primarily from the CLI as the prompt specifies).
...
**Extended library:** `.antigravity/templates/subagents/` — explore-library, shell-library, browser-library (flows, security, i18n, etc.)
...
- `.antigravity/templates/` — Phase prompt, API route, commit, PR, test, DoD
- `.antigravity/contracts/CONTRACTS.md` — Invariants: org scope, soft delete, QR, auth
- `.antigravity/agents/` — Role personas (planning, security, backend-api, etc.), scenarios (code-review, security-audit), orchestrator
