...
Read **current profile** from `docs/plan/learning/ONE_MAN_MEMORY.md` (currentDomain / currentProfile / currentMindset). When user runs `/man` with no args, report status for that domain and offer to switch. Profiles = code | brand | saas | marketing | business | content | copywrite (optional style suffix e.g. code:ship-fast).
...
|--------|---------|----------|
| `backlog/` | Raw tasks, ideas | ALL_TASKS_BACKLOG.md, quick captures |
| `context/` | Refined initiatives | IDEA_<slug>.md |
| `planning/` | Plans being created | PLAN, PROMPT_*, TASKS per slug |
| `planned/` | Ready for dev | Same bundle |
| `in-progress/` | Active development | Same bundle |
| `done/` | Completed | Same bundle |
...
1. **List:** Scan `docs/plan/backlog/`, `planning/`, `planned/`, `in-progress/`, `done/`. Count items per folder. Identify active plan (in-progress or single planned). Report next phase.
2. **Output format:**
   ```
   Backlog: N | Planning: N | Planned: N | In progress: N | Done: N
...
Run the **interactive wizard** (one step at a time). See `.antigravity/commands-ref/one-man-code-settings.md`. Steps: GitHub (auto branch/commit/push), GitHub auth (PAT), planning flow, MCPs, skills. Persist to `docs/plan/learning/ONE_MAN_CODE_SETTINGS.md`.
...
2. **Scan state:** Git, preflight, backlog, plans in planning/planned/in-progress/, next incomplete phase.
...
1. Execute the recommended action (context: current domain). If code: move planned → in-progress if needed, run `/dev` next phase.
...
2. Move to done/ when last phase complete.
...
- **Progress in chat:** For code domain, output a **text/markdown progress block**: phases with [x]/[ ], ASCII bar (e.g. `Phase 3/8 ████████░░░░░░░░`). Task manager: Backlog / Planning / Planned / In progress / Done with counts.
- **Optional:** Update `docs/plan/learning/ONE_MAN_PROGRESS.md` when phases or tasks change.
...
- **File:** `docs/plan/learning/ONE_MAN_MEMORY.md`. Store: currentDomain/currentProfile/currentMindset, lastContext, preferences, recentInspiration. Updates concise; no secrets.
...
- [ ] Resolve plan from in-progress/ → planned/ → planning/ → execution/
- [ ] If planned/: move to in-progress/ before first phase
...
- [ ] If last phase: move in-progress/ → done/

---

## 12. Refs

- `docs/plan/ONE_MAN_CODE.md` — Code domain spec
- `docs/plan/ONE_MAN_CODE_SAAS.md` — SaaS checklist
- `docs/plan/learning/ONE_MAN_MEMORY.md` — Profile, context, preferences
- `docs/plan/learning/ONE_MAN_PROFILES.md` — Profile list (optional)
- `docs/plan/learning/ONE_MAN_CODE_SETTINGS.md` — Settings (wizard output)
