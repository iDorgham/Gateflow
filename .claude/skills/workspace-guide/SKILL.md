...
| **Core rules** | `.antigravity/rules/00-gateflow-core.mdc`, `01-gateflow-ai-workflow.mdc`, `gateflow-security.mdc` | pnpm, multi-tenant, soft deletes, QR security, auth, slash commands |
| **Contracts** | `.antigravity/contracts/CONTRACTS.md` | Invariants: org scope, soft deletes, QR signing, auth, validation |
| **Master commands** | `.antigravity/workflows/` | `/idea`, `/plan`, `/dev`, `/ship` — see `01-gateflow-ai-workflow.mdc` |
| **Internal flows** | `.antigravity/commands-ref/` | ready, github, run, automate, clis (used by `/dev` and `/ship`) |
| **Templates** | `.antigravity/templates/` | e.g. `TEMPLATE_PROMPT_phase.md` for phase prompts |
| **Agents** | `.antigravity/agents/` | orchestrator, roles, scenarios — subagent hierarchy |
...
## 3. Post-task (REQUIRED after every task)

**After** finishing the user’s task, you **MUST** provide a concise guide block and Next Step Injection:
...
- **Next Step Injection** — A copy-pasteable `/dev` command for the next step.

One line per category is required.
...
2. **Assess state**
   ...
   - **Ralph Perspective**: Run `node scripts/ralph-prioritize.js` and `node scripts/ralph-skill-discover.js`.
...
4b. **Context-aware suggestions (plan lifecycle)**  
   When assessing state, adapt suggestions to plan state:
   - **Planning** — If scope unclear: suggest running **explore** subagent to map flows; load gf-planner. If simple: skip to Generate.
   - **Planned** — Next: run `/dev` or `/plan ready <slug>` if still in planning.
   - **In progress** — Next: run `/dev` for next phase. Suggest **MCP** (Prisma-Local if schema, Context7 if docs, cursor-ide-browser if E2E), **Skills** for phase role (gf-security, gf-database, gf-api), **Commands** (/ready, /github).
   - **Blocked / scope creep** — Suggest escalation: break into smaller phases, add INVESTIGATE step, or run `/clis team audit` for review.
   - **Post-phase** — Suggest: update TASKS, run `/github`, optionally adversarial review for high-risk phases. See `docs/plan/PLANNING_ENHANCEMENTS.md`.
   - **After a task done with a CLI**: remind or ensure one entry is added to **`docs/plan/learning/CLI_USAGE_AND_RESULTS.md`** (date, CLI, task, outcome, notes). If the user has indicated "near limit" in GUIDE_PREFERENCES or CLI_LIMITS_TRACKING, prefer free-tier CLIs when suggesting the next tool (load **gf-cli-limits**).
...
- **TASKS file** — When finishing a phase, `TASKS_<plan>.md` (in the same folder as the plan: in-progress/ or done/) must be updated in the same pass as the commit (tick items, set Status: Done). This keeps /guide and /dev accurate. Do this automatically; do not commit without it.
...
- **Code quality** — Lint and typecheck pass; point to `CONTRACTS.md` and `.antigravity/rules/` for invariants.
...
- **Workflows**: Pre-flight and post-task behavior can be referenced from `.antigravity/commands-ref/` or docs so that `/dev` and `/ship` stay aligned with the guide.
...
- Motion: specific animation specs (duration, easing, prefers-reduced-motion).
```
