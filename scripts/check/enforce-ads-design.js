...
function scanDir(dir, violations = []) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      if (file.includes('tokens') || file.includes('tailwind.config') || file.includes('globals.css')) continue;
...
const scanPaths = args.length > 0 ? args.map(a => path.resolve(process.cwd(), a)) : [process.cwd()];
const violations = [];
...
if (violations.length > 0) {
  ...
  console.log('\x1b[36m%s\x1b[0m', `Found ${violations.length} files with hardcoded primitives or raw colors.\n`);
  
  violations.forEach((v) => {
    console.log(`\x1b[33m${v.file}\x1b[0m:`);
    if (v.hex.length) console.log(`  - \x1b[31mHex:\x1b[0m ${v.hex.join(', ')}`);
    if (v.rgba.length) console.log(`  - \x1b[31mRGBA:\x1b[0m ${v.rgba.join(', ')}`);
    if (v.gf.length) console.log(`  - \x1b[35mPrimitive (--gf-*):\x1b[0m ${v.gf.join(', ')}`);
  });
