...
for (const entry of proof.entries) {
  ...
  if (entry.artifact) {
    ...
    if (fs.existsSync(artifactPath)) {
      ...
      entry.artifactSha256 = crypto.createHash('sha256').update(content).digest('hex');
    }
  }
}

fs.writeFileSync(proofPath, JSON.stringify(proof, null, 2) + '\n');
console.log('Successfully refreshed .ai/runtime-proof.json timestamp and SHA256 hashes.');
