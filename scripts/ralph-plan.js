...
const TOOLS = ['claude', 'gemini', 'opencode', 'kilo', 'qwen', 'cursor', 'kiro'];

// ── helpers ───────────────────────────────────────────────────────────────────
function stateDir(state) { return path.join(PLAN_ROOT, state); }
...
switch (cmd) {
  ...
  case 'new': {
    ...
    if (!slug) { console.error('Usage: ralph-plan new <slug> [--phases 5] [--title "Title"]'); process.exit(1); }
    ...
    if (fs.existsSync(dest)) { console.error(`✗ Plan "${slug}" already exists in planning/`); process.exit(1); }

    fs.mkdirSync(dest, { recursive: true });

    // PLAN file
    fs.writeFileSync(planFile(dest, slug), buildPlanTemplate(slug, featureTitle, phases));
    console.log(`✓ Created: docs/plan/planning/${slug}/PLAN_${slug}.md`);

    // PROMPT files
    for (let i = 1; i <= phases; i++) {
      const tool = i === 1 ? 'claude' : i % 3 === 0 ? 'gemini' : 'claude';
      fs.writeFileSync(promptFile(dest, slug, i), buildPromptTemplate(slug, i, `Phase ${i}: Title`, tool));
    ...
    if (fs.existsSync(backlog)) {
      ...
      const phasesEntry = Array.from({ length: phases }, (_, i) =>
        `- [ ] Phase ${i + 1} — Title`
      ).join('\n');
      const entry = `\n### ${slug} — ${featureTitle}\n\n**Status:** 🆕 Open\n\n**Target:** Q4 2026\n\n${phasesEntry}\n`;
      content = content.replace('## Open Initiatives\n', `## Open Initiatives\n${entry}`);
  ...
  case 'ready': {
    ...
    if (!slug) { console.error('Usage: ralph-plan ready <slug>'); process.exit(1); }
  ...
  case 'start': {
    const slug = rawArgs[0];
    if (!slug) { console.error('Usage: ralph-plan start <slug>'); process.exit(1); }
  ...
  case 'done': {
    const slug = rawArgs[0];
    if (!slug) { console.error('Usage: ralph-plan done <slug>'); process.exit(1); }
    const found = findPlan(slug);
    if (!found) { console.error(`✗ Plan "${slug}" not found`); process.exit(1); }
  ...
  case 'move': {
    ...
    if (!slug || !from || !to) { console.error('Usage: ralph-plan move <slug> <from> <to>'); process.exit(1); }
  ...
  case 'status': {
    ...
    for (const state of [...activeStates, 'done']) {
      ...
      const icon = { planning: '📝', planned: '📌', 'in-progress': '🔄', done: '✅' }[state] || '•';
      console.log(`${icon}  ${state.toUpperCase()}`);

      for (const slug of entries) {
        const pf = planFile(path.join(dir, slug), slug);
        if (!fs.existsSync(pf)) { console.log(`   • ${slug}`); continue; }
        ...
        console.log(`   • ${slug.padEnd(35)} ${bar} ${done}/${total} phases (${pct}%)`);
      }
      console.log();
    }
