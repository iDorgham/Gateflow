...
// ── helpers ───────────────────────────────────────────────────────────────────
function readPkg()         { return JSON.parse(fs.readFileSync(PKG, 'utf8')); }
function writePkg(obj)     { fs.writeFileSync(PKG, JSON.stringify(obj, null, 2) + '\n'); }
function current()         { return readPkg().version || '0.1.0'; }
...
function git(cmd, opts = {}) {
  try {
    return execSync(cmd, { cwd: ROOT, stdio: opts.silent ? 'pipe' : 'inherit', encoding: 'utf8' }).trim();
  } catch (e) {
...
function currentBranch() { return git('git branch --show-current', { silent: true }); }
function latestTag()     { return git('git describe --tags --abbrev=0 2>/dev/null', { safe: true, silent: true }); }
function commitCount()   { return git('git rev-list --count HEAD 2>/dev/null', { safe: true, silent: true }) || '0'; }
...
switch (cmd) {
  ...
  case 'branch': {
    const slug   = arg1;
    if (!slug) { console.error('Usage: ralph-version branch <slug>'); process.exit(1); }
